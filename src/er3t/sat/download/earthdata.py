...
def format_satname(satellite, instrument):
    ...
    elif instrument.lower() == "viirs" and (
        satellite.lower() in ["noaa20", "snpp", "noaa-20", "s-npp", "noaa21", "noaa-21"]
    ):
        instrument = instrument.upper()
        satellite = satellite.replace("-", "").upper()
    elif instrument.lower() == "abi" and (
        satellite.lower() in ["goes-east", "goes-west"]
    ):
        instrument = instrument.upper()
        satellite = satellite.upper().replace("WEST", "West").replace("EAST", "East")
    else:
        msg = f"\nError [format_satname]: Currently do not support <{instrument}> onboard <{satellite}>."
        raise NameError(msg)
    # ╰────────────────────────────────────────────────────────────────────────────╯#
...
def get_token_earthdata():
    """Return the Earthdata token configured for the current process."""

    try:
        return os.environ["EARTHDATA_TOKEN"]
    except KeyError as error:
        raise RuntimeError(
            "EARTHDATA_TOKEN is required for token-authenticated downloads. "
            "Create a token at https://urs.earthdata.nasa.gov/ and set it in "
            "the environment before running the download command."
        ) from error
...
def get_command_earthdata(
    ...
    primary_tool="curl",
    backup_tool="wget",
    fdir_save=f"{fdir_data_tmp}/satfile",
    verbose=False,
):
    """
    ...
    if token_mode:  # recommended
        ...
        if verbose:
            options = {
                "curl": f'--header {header} --connect-timeout 60.0 --retry 1 --max-time 60.0 --location --continue-at - --output "{fname_save}" "{fname_target}"',
                "wget": f'--header={header} --continue --timeout=60 --tries=2 --show-progress --output-document="{fname_save}" "{fname_target}"',
            }
        else:
            options = {
                "curl": f'-sS --no-progress-bar --header {header} --connect-timeout 60.0 --max-time 60.0 --retry 1 --location --continue-at - --output "{fname_save}" "{fname_target}"',
                "wget": f'--header={header} --continue --timeout=60 --tries=2  --quiet --output-document="{fname_save}" "{fname_target}"',
            }
        ...
        if verbose:
            options = {
                "curl": '--netrc --cookie-jar {} --cookie {} --connect-timeout 60.0 --max-time 60.0 --retry 1 --location --continue-at - --output "{}" "{}"'.format(
                    secret["cookies"], secret["cookies"], fname_save, fname_target
                ),
                "wget": '--continue --load-cookies={} --save-cookies={} --auth-no-challenge --keep-session-cookies --content-disposition --timeout=60 --tries=2 --show-progress --output-document="{}" "{}"'.format(
                    secret["cookies"], secret["cookies"], fname_save, fname_target
                ),
            }
        else:
            options = {
                "curl": '-s --netrc --cookie-jar {} --cookie {} --connect-timeout 60.0 --max-time 60.0 --retry 1 --location --continue-at - --output "{}" "{}"'.format(
                    secret["cookies"], secret["cookies"], fname_save, fname_target
                ),
                "wget": '--continue --load-cookies={} --save-cookies={} --auth-no-challenge --quiet --keep-session-cookies --content-disposition --timeout=60 --tries=2 --output-document="{}" "{}"'.format(
                    secret["cookies"], secret["cookies"], fname_save, fname_target
                ),
            }
...
def get_fname_geometa(
    ...
    if server == "https://ladsweb.modaps.eosdis.nasa.gov":
        fnames_geometa = {
            ...
            "NOAA21|VIIRS": f"{server}/archive/geoMetaVIIRS/5200/NOAA-21/{date.year:04d}/VJ203MOD_{date_s}.txt",
        }
    # ╰────────────────────────────────────────────────────────────────────────────╯#
    ...
    # ╭────────────────────────────────────────────────────────────────────────────╮#
        ...
        fnames_geometa = {
            ...
            "NOAA21|VIIRS": f"{server}/api/v2/content/archives/geoMetaVIIRS/5200/NOAA-21/{date.year:04d}/VJ203MOD_NRT_{date_s}.txt",
        }
    else:
...
def get_local_file(
    ...
    fname_local1 = os.path.abspath(f"{fdir_local}/{filename}")
    fname_local2 = os.path.abspath(f"{fdir_save}/{filename}")

    if os.path.exists(fname_local1):
        os.system(f"cp {fname_local1} {fname_local2}")
...
def get_online_file(
    ...
    if download:
        ...
        try:
            ...
            if (geometa is True) or (
                csv is True
            ):  # they can be None so make True explicit
            ...
            try:
                ...
                try:
                    ...
                    # er3t.common.logger.info("Executing following operation as a backup...\n{}".format(backup_command))
...
def get_nsidc_file_list(
    ...
    # build search params
    temporal = (
        start_dt_hhmm.strftime("%Y-%m-%dT%H:%M:%SZ")
        + ","
        + end_dt_hhmm.strftime("%Y-%m-%dT%H:%M:%SZ")
    )
    bbox = (
        f"{extent[0]},{extent[2]},{extent[1]},{extent[3]}"  # bottom lonlat, top lonlat
    )
    search_params = dict(
    ...
    cmr_collections_url = (
        "https://cmr.earthdata.nasa.gov/search/collections.json?has_granules=True"
    )
    search_params_string = "&".join(f"{k}={v}" for (k, v) in search_params.items())
    ...
    if response.status_code != 200:
        er3t.common.logger.info(
            f"Message [get_nsidc_file_list]: Could not submit query to {cmr_collections_url} as it resulted in a status code {response.status_code}"
        )
        return []
    ...
    if len(results) == 0:
        er3t.common.logger.info(
            f"Message [get_nsidc_file_list]: Could not find any results on {cmr_collections_url}"
        )
        return []
    # er3t.common.logger.info('Message [get_nsidc_file_list]: Found dataset {}'.format(results[0]['dataset_id'])) # debug statement
    ...
    if nsidc_counts == 0:
        er3t.common.logger.info(
            "Message [get_nsidc_file_list]: Could not find any data on NSIDC data centers"
        )
        return []
    ...
    cmr_granules_base_url = (
        "https://cmr.earthdata.nasa.gov/search/granules.json?downloadable=True"
    )
    # update search params with page info
    search_params["page_num"] = 1
    search_params["page_size"] = (
        10  # do not make this too big as query will take too long
    )
    ...
    while True:
        ...
        if response.status_code != 200:
            er3t.common.logger.info(
                f"Message [get_nsidc_file_list]: Could not submit query to {cmr_granules_url} as it resulted in a status code {response.status_code}"
            )
            return []
    ...
    er3t.common.logger.info(
        f"Message [get_nsidc_file_list]: Found {len(nsidc_download_links)} granules"
    )
...
def read_geometa(content):
    ...
    if len(lines) == 1:
        ...
        if (lines[0][0] == "{") and (lines[0][-1] == "}"):
            msg = (
                msg
                + " or the date for which you are looking to download does not have data.\n"
            )
...
def cal_proj_xy_geometa(line_data, closed=True):
    ...
    if (
        (abs(lon_[0] - lon_[1]) > 180.0)
        | (abs(lon_[0] - lon_[2]) > 180.0)
        | (abs(lon_[0] - lon_[3]) > 180.0)
        | (abs(lon_[1] - lon_[2]) > 180.0)
        | (abs(lon_[1] - lon_[3]) > 180.0)
        | (abs(lon_[2] - lon_[3]) > 180.0)
    ):
        lon_[lon_ < 0.0] += 360.0
    # ╰────────────────────────────────────────────────────────────────────────────╯#
    ...
    proj_xy_ = ccrs.Orthographic(
        central_longitude=center_lon_, central_latitude=center_lat_
    )
...
def cal_lon_lat_utc_geometa(
    ...
    if testing:
        ...
        fig.savefig(f"globe-view_{fname_png}", bbox_inches="tight", metadata=_metadata)
        er3t.common.logger.info(
            f"\nMessage [cal_lon_lat_utc_geometa]: Figure saved as 'globe-view_{fname_png}'"
        )
        # ╰──────────────────────────────────────────────────────────────╯#
    # ╰────────────────────────────────────────────────────────────────────────────╯#
...
def cal_sec_offset_abi(extent, satname="GOES-East|ABI", sec_per_scan=30.0):
    ...
    xy = proj_xy.transform_points(proj_lonlat, lon_scan_edges, lat_scan_edges)[
        :, [0, 1]
    ]
    ...
    xy_in = proj_xy.transform_points(proj_lonlat, lon_in.ravel(), lat_in.ravel())[
        :, [0, 1]
    ]
    x_in = xy_in[:, 0]
    y_in = xy_in[:, 1]
    ...
    for i in range(Ns, Ne):
        ...
        if span_time0 is not None:
            ...
            if logic_in.sum() > 0:
                ...
                delta_scan_x_half = R_earth * np.sin(
                    np.arccos(((y_edge_min + y_edge_max) / 2.0) / R_earth)
                )
...
def get_satfile_tag(
    ...
    filename_geometa = "{}_{}".format(
        server.replace("https://", "").split(".")[0], os.path.basename(fname_geometa)
    )

    # try to get geometa information from local
    content = get_local_file(
        fname_geometa,
        filename=filename_geometa,
        fdir_local=fdir_local,
        fdir_save=fdir_save,
    )

    # try to get geometa information online
    if (content is None) or ("<!DOCTYPE html>" in content):
        content = get_online_file(
            fname_geometa,
            geometa=True,
            csv=None,
            filename=filename_geometa,
    ...
    for i in range(Ndata):
        ...
        if (
            (Npoint_in > 0)
            and (line["DayNightFlag"] == "D")
            and (percent_in >= percent0)
        ):
            if geometa:
                filename_tags.append(line)
            else:
                granule_dt = line["StartDateTime"]
                granule_dt = datetime.datetime.strptime(
                    granule_dt, "%Y-%m-%d %H:%M"
                )  # format it for processing

                if (
                    start_dt_hhmm <= granule_dt <= end_dt_hhmm
                ):  # get the filename only if within time bounds
...
def download_laads_https(
    ...
    # ╭────────────────────────────────────────────────────────────────────────────╮#
    exist_count = (
        0  # to prevent re-downloading. TODO: Add `overwrite` option instead for user
    )
    lines = content.split("\n")
    primary_commands = []
    backup_commands = []
    ...
    for line in lines:
        ...
        if filename_tag in filename:
            ...
            if os.path.isfile(fname_local) and final_file_check(
                ...
                er3t.common.logger.info(
                    f"Message [download_laads_https]: File {fname_local} already exists and looks good. Will not re-download this file."
                )
                exist_count += 1
            else:
                fnames_local.append(fname_local)
                primary_command, backup_command = get_command_earthdata(
                    fname_server, filename=filename, fdir_save=fdir_out, verbose=verbose
                )
                primary_commands.append(primary_command)
                backup_commands.append(backup_command)

    er3t.common.logger.info(
        f"Message [download_laads_https]: Total of {len(fnames_local)} will be downloaded. {exist_count} will be skipped as they already exist and work as advertised."
    )
    # ╰────────────────────────────────────────────────────────────────────────────╯#
        ...
        for i in range(len(primary_commands)):
            ...
            if not final_file_check(
                ...
                if not final_file_check(
                    ...
                    er3t.common.logger.info(
                        f"Message [download_laads_https]: Could not complete the download of or something is wrong with {fname_local}...deleting..."
                    )
...
def download_lance_https(
    ...
    for line in lines:
        ...
        if (filename_tag in filename) and (".met" not in filename):
            ...
            if os.path.isfile(fname_local) and final_file_check(
                ...
                er3t.common.logger.info(
                    f"Message [download_lance_https]: File {fname_local} already exists and looks good. Will not re-download this file."
                )
                exist_count += 1
            else:
                fnames_local.append(fname_local)
                ...
                backup_commands.append(
                    "timeout 60 " + backup_command
                )  # force timeout for wget

    er3t.common.logger.info(
        f"Message [download_lance_https]: Total of {len(fnames_local)} will be downloaded. {exist_count} will be skipped as they already exist and work as advertised."
    )
    # ╰────────────────────────────────────────────────────────────────────────────╯#
    ...
    if run:
        for i in range(len(primary_commands)):
            ...
            if verbose:
                er3t.common.logger.info(
                    f"Message [download_lance_https]: Downloading {fname_local} ..."
                )
            os.system(primary_commands[i])
            ...
            if not final_file_check(
                ...
                if not final_file_check(
                    ...
                    er3t.common.logger.info(
                        f"Message [download_lance_https]: Could not complete the download of or something is wrong with {fname_local}...deleting..."
                    )
                    os.remove(fname_local)
                    fnames_local.remove(fname_local)  # remove from list
    else:
        er3t.common.logger.info(
            "Message [download_lance_https]: The commands to run are:"
        )
        for command in primary_commands:
            er3t.common.logger.info(command)
    # ╰────────────────────────────────────────────────────────────────────────────╯#
...
def download_nsidc_https(
    ...
    files = get_nsidc_file_list(
        product_id=product_dict["short_name"],
    ...
    for fname_server in files:
        ...
        if has_common_substring(
            filename, filename_tags
        ):  # if filename contains any match to the tags, proceed with downloading
            fname_local = f"{fdir_out}/{filename}"
            if os.path.isfile(fname_local) and final_file_check(
                fname_local, data_format=data_format, verbose=verbose
            ):
                er3t.common.logger.info(
                    f"Message [download_nsidc_https]: File {fname_local} already exists and looks good. Will not re-download this file."
                )
                exist_count += 1
            else:
                fnames_local.append(fname_local)
    ...
    er3t.common.logger.info(
        f"Message [download_nsidc_https]: Total of {len(fnames_local)} will be downloaded. {exist_count} will be skipped as they already exist and work as advertised."
    )
    # \----------------------------------------------------------------------------/#
    ...
    if run:
        for i in range(len(primary_commands)):
            ...
            if verbose:
                er3t.common.logger.info(
                    f"Message [download_nsidc_https]: Downloading {fname_local} ..."
                )
            os.system(primary_commands[i])
            ...
            if not final_file_check(
                ...
                if not final_file_check(
                    ...
                    er3t.common.logger.info(
                        f"Message [download_nsidc_https]: Could not complete the download of or something is wrong with {fname_local}...deleting..."
                    )
                    os.remove(fname_local)
                    fnames_local.remove(fname_local)  # remove from list
    else:
        er3t.common.logger.info(
            "Message [download_nsidc_https]: The commands to run are:"
        )
        for command in primary_commands:
            er3t.common.logger.info(command)
    # \----------------------------------------------------------------------------/#
...
def download_oco2_https(
    ...
    from er3t.sat.readers.oco2 import get_fnames_from_web, get_dtime_from_xml
    ...
    if fnames is not None:
        ...
        for i in range(Nfile):
            dtime_s, dtime_e = get_dtime_from_xml(f"{fdir_server}/{fnames_xml[i]}")
...
def download_worldview_image(
    ...
    if satellite in ["Aqua", "Terra", "NOAA20", "SNPP", "NOAA21"]:
        ...
        try:
            ...
            if satellite in ["Aqua", "Terra"]:
                lon0_, lat0_, jday0_ = cal_lon_lat_utc_geometa(
                    line_data,
                    delta_t=300.0,
                    N_along=1015,
                    N_cross=677,
            ...
            fname = "{}/{}-{}_{}_{}_({}).png".format(
                ...
                date_s0,
                ",".join([f"{extent0:.4f}" for extent0 in extent]),
            )
        ...
        date = (
            ...
            + round((date - datetime.datetime.min) / delta) * delta
        )
    ...
    if run:
        ...
        try:
            ...
            if coastline:
                ax1.coastlines(
                    resolution="10m", color="black", linewidth=0.5, alpha=0.8
                )
