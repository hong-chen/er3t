...
import er3t.common
from er3t.core._utilities import get_data_h4, get_data_nc, unpack_uint_to_bits
from .modis import cal_sinusoidal_grid
...
VIIRS_L1B_IMG_BANDS = {"I01": 640, "I02": 865, "I03": 1610, "I04": 3740, "I05": 11450}


VIIRS_L1B_MOD_EMISSIVE_BANDS = {
    "M12": 3700,
    "M13": 4050,
    "M14": 8550,
    "M15": 10763,
...
VIIRS_L1B_IMG_EMISSIVE_BANDS = {"I04": 3740, "I05": 11450}

VIIRS_L1B_ALL_EMISSIVE_BANDS = {
    "M12": 3700,
    "M13": 4050,
    "M14": 8550,
    "M15": 10763,
...
VIIRS_L1B_MOD_DEFAULT_BANDS = {"M05": 673, "M04": 555, "M02": 445}
...
class viirs_03:
    ...
    def __init__(
        self, fnames=None, extent=None, vnames=[], keep_dims=False, verbose=False
    ):
        ...
        for vname in vnames or ():
            ...
            if vname.lower() in self.data.keys():
                self.data[vname.lower()] = dict(
                    name=vname.lower().title(),
                    data=np.vstack((self.data[vname.lower()]["data"], data)),
                    units=data0.getncattr("units"),
                )
            else:
                self.data[vname.lower()] = dict(
                    name=vname.lower().title(),
...
class viirs_l1b:
    ...
    def __init__(
        ...
        self.keep_dims = (
            keep_dims  # retain 2D shape; if True -> f03 mask is not applied
        )
        self.auto_mask = (
            auto_mask  # apply mask to flagged data such as data due to bow-tie effect
        )
    ...
    def _mask_flags(self, nc_dset, fill_value=np.nan):
        ...
        data = np.ma.masked_array(
            data.data, data.mask, fill_value=fill_value, dtype="float"
        )
        flags = nc_dset.getncattr("flag_values")
    ...
    def read(self, fname):
        ...
        for i in range(len(self.bands)):
            ...
            if hasattr(nc_dset, "radiance_add_offset"):
                rad0 = (
                    data - nc_dset.getncattr("radiance_add_offset")
                ) * nc_dset.getncattr("radiance_scale_factor")  # radiance
                ref[i] = (data - nc_dset.getncattr("add_offset")) * nc_dset.getncattr(
                    "scale_factor"
                )  # reflectance

            else:  # naming convention changes for emissive bands
                rad0 = (data - nc_dset.getncattr("add_offset")) * nc_dset.getncattr(
                    "scale_factor"
                )  # radiance
                ref[i] = np.full(
                    ref[i].shape, -99, dtype="int8"
                )  # make reflectance -99 for emissive bands

            rad0 /= 1000.0  # from <per micron> to <per nm>
            rad[i] = rad0
...
class viirs_cldprop_l2:
    ...
    def __init__(
        ...
        self.keep_dims = (
            keep_dims  # flag; if false -> convert to 1D; true -> retain 2D granule data
        )
    ...
    def extract_data(self, dbyte, byte=0):
        ...
        if byte == 0:
            ...
            if self.keep_dims:
                return (
                    cloud_mask_flag.reshape(self.data_shape),
                    day_night_flag.reshape(self.data_shape),
                    sunglint_flag.reshape(self.data_shape),
                    snow_ice_flag.reshape(self.data_shape),
                    land_water_cat.reshape(self.data_shape),
                    fov_cat.reshape(self.data_shape),
                )
            return (
                cloud_mask_flag,
                day_night_flag,
    ...
    def quality_assurance_byte0(self, dbyte):
        ...
        ret_1621_conf = (
            2 * data[:, 1] + 1 * data[:, 2]
        )  # convert to a value between 0 and 3 confidence
        ...
        if self.keep_dims:
            return (
                ret_std.reshape(self.data_shape),
                ret_std_conf.reshape(self.data_shape),
                ret_std_data.reshape(self.data_shape),
                ret_1621.reshape(self.data_shape),
                ret_1621_conf.reshape(self.data_shape),
                ret_1621_data.reshape(self.data_shape),
            )
        return (
            ret_std,
            ret_std_conf,
    ...
    def quality_assurance_byte1(self, dbyte):
        ...
        if self.keep_dims:
            return (
                cld_type_process.reshape(self.data_shape),
                rayleigh.reshape(self.data_shape),
                cot_bands.reshape(self.data_shape),
                cot_oob.reshape(self.data_shape),
                bowtie.reshape(self.data_shape),
            )
    ...
    def read_mask(self, fname):
        ...
        cm0 = cm[
            :, :, 0
        ]  # read only the first byte; rest will be supported in the future if needed
        ...
        if (self.quality_assurance is not None) and (self.quality_assurance > 0):
            ...
            qa0 = qa[
                :, :, 0
            ]  # read byte 0 for confidence QA (note that indexing is different from MODIS)
            qa1 = qa[
                :, :, 1
            ]  # read byte 1 for confidence QA (note that indexing is different from MODIS)
            ...
            (
                ...
                ret_1621_conf_qa,
                ret_1621_data_qa,
            ) = self.quality_assurance_byte0(qa0)
            cld_type_qa, rayleigh_qa, cot_bands_qa, cot_oob_qa, bowtie_qa = (
                self.quality_assurance_byte1(qa1)
            )
        ...
        if hasattr(self, "data"):
            ...
            self.data["fov_cat"] = dict(
                name="FOV quality cateogry",
                data=np.hstack((self.data["fov_cat"]["data"], fov_cat)),
                units="N/A",
            )
            ...
            self.data["fov_cat"] = dict(
                name="FOV quality category", data=fov_cat, units="N/A"
            )
            self.data["day_night_flag"] = dict(
        ...
        if (self.quality_assurance is not None) and (self.quality_assurance > 0):
            if hasattr(self, "qa"):
                self.qa["ret_std_conf_qa"] = dict(
                    name="QA standard retrieval confidence",
                    data=np.hstack(
                        (self.qa["ret_std_conf_qa"]["data"], ret_std_conf_qa)
                    ),
                    units="N/A",
                )
                self.qa["ret_1621_conf_qa"] = dict(
                    name="QA 1.6-2.1 retrieval confidence",
                    data=np.hstack(
                        (self.qa["ret_1621_conf_qa"]["data"], ret_1621_conf_qa)
                    ),
                    units="N/A",
                )

                if self.quality_assurance > 1:
                    self.qa["ret_std_qa"] = dict(
                        name="QA standard retrieval outcome",
                        data=np.hstack((self.qa["ret_std_qa"]["data"], ret_std_qa)),
                        units="N/A",
                    )
                    self.qa["ret_std_qa_data"] = dict(
                        name="QA standard retrieval spectral data availability",
                        data=np.hstack(
                            (self.qa["ret_std_data_qa"]["data"], ret_std_data_qa)
                        ),
                        units="N/A",
                    )
                    self.qa["ret_1621_qa"] = dict(
                        name="QA 1621 retrieval outcome",
                        data=np.hstack((self.qa["ret_1621_qa"]["data"], ret_1621_qa)),
                        units="N/A",
                    )
                    self.qa["ret_1621_data_qa"] = dict(
                        name="QA 1621 retrieval spectral data availability",
                        data=np.hstack(
                            (self.qa["ret_1621_data_qa"]["data"], ret_1621_data_qa)
                        ),
                        units="N/A",
                    )
                    self.qa["cld_type_qa"] = dict(
                        name="QA cloud type processing path",
                        data=np.hstack((self.qa["cld_type_qa"]["data"], cld_type_qa)),
                        units="N/A",
                    )

                    self.qa["bowtie_qa"] = dict(
                        name="QA bowtie pixel",
                        data=np.hstack((self.qa["bowtie_qa"]["data"], bowtie_qa)),
                        units="N/A",
                    )

                    self.qa["rayleigh_qa"] = dict(
                        name="QA Rayleigh correction",
                        data=np.hstack((self.qa["rayleigh_qa"]["data"], rayleigh_qa)),
                        units="N/A",
                    )

                    self.qa["cot_bands_qa"] = dict(
                        name="QA bands used for COT",
                        data=np.hstack((self.qa["cot_bands_qa"]["data"], cot_bands_qa)),
                        units="N/A",
                    )

                    self.qa["cot_oob_qa"] = dict(
                        name="QA COT out of bounds",
                        data=np.hstack((self.qa["cot_oob_qa"]["data"], cot_oob_qa)),
                        units="N/A",
                    )
                ...
                self.qa["ret_std_conf_qa"] = dict(
                    name="QA standard retrieval confidence",
                    data=ret_std_conf_qa,
                    units="N/A",
                )
                self.qa["ret_1621_conf_qa"] = dict(
                    name="QA 1.6-2.1 retrieval confidence",
                    data=ret_1621_conf_qa,
                    units="N/A",
                )
                self.qa["ret_std_qa"] = dict(
                    name="QA standard retrieval outcome", data=ret_std_qa, units="N/A"
                )
                self.qa["ret_std_data_qa"] = dict(
                    name="QA standard retrieval spectral data availability",
                    data=ret_std_data_qa,
                    units="N/A",
                )
                self.qa["ret_1621_data_qa"] = dict(
                    name="QA 1621 retrieval spectral data availability",
                    data=ret_1621_data_qa,
                    units="N/A",
                )
                ...
                if self.quality_assurance > 1:
                    ...
                    self.qa["bowtie_qa"] = dict(
                        name="QA bowtie pixel", data=bowtie_qa, units="N/A"
                    )
                    self.qa["rayleigh_qa"] = dict(
                        name="QA Rayleigh correction", data=rayleigh_qa, units="N/A"
                    )
                    self.qa["cot_bands_qa"] = dict(
                        name="QA bands used for COT", data=cot_bands_qa, units="N/A"
                    )
                    self.qa["cot_oob_qa"] = dict(
                        name="QA COT out of bounds", data=cot_oob_qa, units="N/A"
                    )
    ...
    def read_cop(self, fname):
        ...
        # -------------------------------------Uncertainties-------------------------------------#
        cot_uct0 = f.groups["geophysical_data"].variables[
            "Cloud_Optical_Thickness_Uncertainty"
        ]
        cer_uct0 = f.groups["geophysical_data"].variables[
            "Cloud_Effective_Radius_Uncertainty"
        ]
        cwp_uct0 = f.groups["geophysical_data"].variables[
        ...
        if hasattr(self, "data"):
            ...
            self.data["ctp"] = dict(
                name="Cloud phase optical proprties",
                data=np.hstack((self.data["ctp"]["data"], ctp)),
                units="N/A",
            )
            self.data["cth"] = dict(
                name="Cloud top height",
                data=np.hstack((self.data["cth"]["data"], cth)),
                units="m",
            )
            self.data["cot"] = dict(
            ...
            self.data["cot_uct"] = dict(
                name="Cloud optical thickness uncertainty",
                data=np.hstack((self.data["cot_uct"]["data"], cot * cot_uct / 100.0)),
                units="N/A",
            )
            self.data["cer_uct"] = dict(
                name="Cloud effective radius uncertainty",
                data=np.hstack((self.data["cer_uct"]["data"], cer * cer_uct / 100.0)),
                units="micron",
            )
            self.data["cwp_uct"] = dict(
                name="Cloud water path uncertainty",
                data=np.hstack((self.data["cwp_uct"]["data"], cwp * cwp_uct / 100.0)),
                units="g/m^2",
            )
            self.data["pcl"] = dict(
                name="PCL tag (1:PCL)",
            ...
            self.data["lat"] = dict(name="Latitude", data=lat, units="degrees")
            self.data["ctp"] = dict(
                name="Cloud phase optical properties", data=ctp, units="N/A"
            )
            ...
            self.data["cot_uct"] = dict(
                name="Cloud optical thickness uncertainty",
                data=cot * cot_uct / 100.0,
                units="N/A",
            )
            self.data["cer_uct"] = dict(
                name="Cloud effective radius uncertainty",
                data=cer * cer_uct / 100.0,
                units="micron",
            )
            self.data["cwp_uct"] = dict(
                name="Cloud water path uncertainty",
                data=cwp * cwp_uct / 100.0,
                units="g/m^2",
            )
            self.data["pcl"] = dict(name="PCL tag (1:PCL)", data=pcl, units="N/A")


class viirs_mvcm_cldmsk_l2:
    """
    A class for extracting data from VIIRS/SNPP, VIIRS/NOAA-20, and VIIRS/NOAA-21 Cloud Mask 6-Min Swath 750 m files (CLDMSK_L2).
    ...
    Note: This class is the same as the MODIS version `class modis_mvcm_cldmsk_l2`
    """

    ID = "VIIRS MVCM Continuity Cloud Mask 6-Min Swath 750 m"
...
class viirs_09:
    ...
    def __init__(
        ...
        self.bands = list(
            map(str.upper, bands)
        )  # list of band names; check VIIRS_ALL_BANDS for convention

        self.available_product_bands = [
            "I01",
            "I02",
            "I03",
            "M01",
    ...
    def qa_qf2_viirs_09(self, hdf_obj):
        ...
        return (
            ...
            heavy_aerosol_qa,
            snow_ice_qa,
            thin_cirrus_solar_qa,
            thin_cirrus_thermal_qa,
        )
    ...
    def extract_surface_reflectance(self, hdf_obj):
        ...
        if self.resolution == "375m":
            ...
            raise AttributeError(
                "Error [viirs_09]: `resolution` must be one of `375m` or `750m`"
            )
        ...
        if self.quality_assurance is not None:
            ...
            if (self.quality_assurance.lower() == "auto") or (
                self.quality_assurance.lower() == "all"
            ):
                (
                    overall_m8_qa,
                    overall_m10_qa,
                ...
                (
                    ...
                    overall_m5_qa,
                    overall_m7_qa,
                ) = self.qa_qf5_viirs_09(hdf_obj)

                overall_ibands_qa = np.stack(
                    [overall_i1_qa, overall_i2_qa, overall_i3_qa], axis=0
                )
                overall_mbands_qa = np.stack(
                    [
                        overall_m1_qa,
                        overall_m2_qa,
                        overall_m3_qa,
                ...
                self.qa["overall_ibands_qa"] = dict(
                    name="Overall Quality of Surf. Refl. of Bands: I1, I2, I3",
                    data=overall_ibands_qa,
                    units="N/A",
                )
                self.qa["overall_mbands_qa"] = dict(
                    name="Overall Quality of Surf. Refl. of Bands: M1, M2, M3, M4, M5, M7, M8, M9, M10, M11",
                    data=overall_mbands_qa,
                    units="N/A",
                )

            if (self.quality_assurance.lower() == "ancillary") or (
                self.quality_assurance.lower() == "all"
            ):
                (
                    cld_msk_qa,
                    cld_msk_confidence_qa,
                    day_night_qa,
                    low_sun_qa,
                    sunglint_qa,
                ) = self.qa_qf1_viirs_09(hdf_obj)
                (
                    land_water_qa,
                    cld_shadow_qa,
                    heavy_aerosol_qa,
                ...
                (
                    ...
                    invalid_land_qa,
                    missing_pw_qa,
                ) = self.qa_qf4_viirs_09(hdf_obj)
                missing_ozone_qa, missing_sfc_press_qa, _, _, _, _, _, _ = (
                    self.qa_qf5_viirs_09(hdf_obj)
                )

                bad_ibands_qa = np.stack([bad_i1_qa, bad_i2_qa, bad_i3_qa], axis=0)
                bad_mbands_qa = np.stack(
                    [
                        bad_m1_qa,
                        bad_m2_qa,
                        bad_m3_qa,
                ...
                missing_data_desc = (
                    "index 0 = missing AOT input data\n"
                ...
                self.qa["missing_data_qa"] = dict(
                    name="Missing or Invalid Input Data QA",
                    description=missing_data_desc,
                    data=missing_data_qa,
                    units="N/A",
                )
                self.qa["ancillary_data_qa"] = dict(
                    name="Ancillary Data QA",
                    description=ancillary_data_desc,
                    data=ancillary_data_qa,
                    units="N/A",
                )
                self.qa["bad_ibands_qa"] = dict(
                    name="Bad L1b/SDR data of Bands: I1, I2, I3",
                    data=bad_ibands_qa,
                    units="N/A",
                )
                self.qa["bad_mbands_qa"] = dict(
                    name="Bad L1b/SDR data of Bands: M1, M2, M3, M4, M5, M7, M8, M9, M10, M11",
                    data=bad_mbands_qa,
                    units="N/A",
                )
    ...
    def read(self, fname):
        try:
            ...
            msg = (
                "Warning [viirs_09]: To use 'viirs_09', 'pyhdf' needs to be installed."
            )
            raise ImportError(msg)
...
class viirs_09a1:
    ...
    def __init__(self, fnames, extent=None, band="M4", Nx=1200, Ny=1200, verbose=False):
        self.fnames = fnames
        self.extent = extent
...
class viirs_43ma3:
    ...
    def __init__(
        self, fnames=None, extent=None, channels=["M4"], Nx=1200, Ny=1200, verbose=False
    ):
        self.fnames = fnames
        self.extent = extent
...
class viirs_43ma4:
    ...
    def read(self, fname, channels=["M4"]):
        ...
        if hasattr(self, "data"):
            ...
            self.data["ref"] = dict(
                name="Surface reflectance at nadir",
                data=np.hstack((self.data["ref"]["data"], sfc_ref)),
                units="N/A",
            )
        else:
            self.data = {}
            ...
            self.data["y"] = dict(name="Y of sinusoidal grid", data=y, units="m")
            self.data["ref"] = dict(
                name="Surface reflectance at nadir", data=sfc_ref, units="N/A"
            )
