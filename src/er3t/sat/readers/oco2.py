...
from er3t.core._utilities import check_equal
...
class oco2_rad_nadir:
    ...
    def overlap(self, index_s=0, index_e=None, lat0=0.0, lon0=0.0):
        ...
        xco2 = np.zeros_like(self.lon_l1b)
        xco2[...] = np.nan
        sfc_pres = np.zeros_like(self.lon_l1b)
        sfc_pres[...] = np.nan
    ...
    def get_data(self, index_s=0, index_e=None):
        ...
        for i in range(8):
            self.rad_o2_a[:, i, :] = convert_photon_unit(
                self.rad_o2_a[:, i, :], self.get_wvl_o2_a(i)
            )
            self.rad_co2_weak[:, i, :] = convert_photon_unit(
                self.rad_co2_weak[:, i, :], self.get_wvl_co2_weak(i)
            )
            self.rad_co2_strong[:, i, :] = convert_photon_unit(
                self.rad_co2_strong[:, i, :], self.get_wvl_co2_strong(i)
            )
        f.close()
...
class oco2_std:
    ...
    def read(self, fname):
        ...
        if hasattr(self, "data"):
            ...
            self.data["xco2"] = dict(
                ...
                data=np.hstack((self.data["xco2"]["data"], xco2)),
                units="N/A",
            )
    ...
    def read_vars(self, fname, vnames=None, resolution="5km"):
        try:
            import h5py
        except ImportError:
            msg = "Warning [oco2_std]: To use 'oco2_std', 'h5py' needs to be installed."
        ...
        for vname in vnames or ():
            ...
            if vname_new in self.data.keys():
                self.data[vname_new] = dict(
                    name=vname, data=np.hstack((self.data[vname_new]["data"], data))
                )
            else:
...
class oco2_met:
    ...
    def __init__(
        self, fnames=None, extent=None, overwrite=False, quiet=True, verbose=False
    ):
        self.fnames = fnames  # file name of the hdf files
        self.extent = (
            extent  # specified region [westmost, eastmost, southmost, northmost]
...
def get_fnames_from_web(website, extension):
    ...
    for fname_web in fnames_web:
        if (fname_web["href"] not in fnames) and (
            fname_web["href"][-len(extension) :] == extension
        ):
...
def get_dtime_from_xml(fname_link):
    ...
    if (
        ("date" in root[2][3].tag.lower())
        and ("time" in root[2][2].tag.lower())
        and ("date" in root[2][1].tag.lower())
        and ("time" in root[2][0].tag.lower())
    ):
    ...
    dtime_begin = datetime.datetime.strptime(
        dtime_begin_str, "%Y-%m-%d %H:%M"
    ) + datetime.timedelta(seconds=seconds_begin)
    dtime_end = datetime.datetime.strptime(
        dtime_end_str, "%Y-%m-%d %H:%M"
    ) + datetime.timedelta(seconds=seconds_end)
