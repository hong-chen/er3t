...
class ahi_l2:
    ...
    def read(self, fname, cop_flag=""):
        ...
        try:
            ...
            msg = "Warning [himawari_l2]: To use 'himawari_l2', 'netCDF4' needs to be installed."
        ...
        logic = (
            ...
            & (lat >= lat_range[0])
            & (lat <= lat_range[1])
            & (np.logical_not(np.ma.getmaskarray(lon)))
            & (np.logical_not(np.ma.getmaskarray(lon)))
            & (np.logical_not(np.ma.getmaskarray(cot)))
            & (np.logical_not(np.ma.getmaskarray(cer)))
        )
        ...
        for vname in vnames or ():
            ...
            if vname.lower() in self.data.keys():
                self.data[vname.lower()] = dict(
                    name=vname, data=np.hstack((self.data[vname.lower()]["data"], data))
                )
            else:
