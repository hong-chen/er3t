...
_description_ = (
    "=" * _width_
    + "\n\n"
    + "Education and Research 3D Radiative Transfer Toolbox (EaR\u00b3T)".center(
        _width_
    )
    + "\n\n"
    + "=" * _width_
...
def satellite_download(
    ...
    if (date is None) and ((start_date is None) or (end_date is None)) and (nrt):
        ...
        start_dt_hhmm = datetime.datetime(
            int(date[:4]), int(date[4:6]), int(date[6:8]), 0, 0
        )
        end_dt_hhmm = datetime.datetime(
            int(date[:4]), int(date[4:6]), int(date[6:8]), 23, 59
        )
        ...
        fdir_out_dt = os.path.join(
            fdir_out, single_dt.strftime("%Y-%m-%d")
        )  # save files in dirs with dates specified
        ...
        if verbose:
            satlogger.info(
                "Message [sdown]: Obtaining data for: {} over following region:\nWest: {}, East: {}, South: {}, North: {}".format(
                    single_dt.strftime("%d %B, %Y"),
                    np.nanmin(llons),
                    np.nanmax(llons),
                    np.nanmin(llats),
        ...
        if (start_date is not None) and (end_date is not None):
            ...
            satlogger.error(
                "Error [sdown]: Please provide a date via --date or date range via --start_date and --end_date or --latest_nhours. Note that --latest_nhours is only valid for near-real time data"
            )
            sys.exit()
        ...
        date_list = [
            start_dt.date() + datetime.timedelta(days=x)
        ...
        if parallel:
            ...
            for date_x in date_list:  # download products date by date
                fdir_out_dt = os.path.join(
                    fdir_out, date_x.strftime("%Y-%m-%d")
                )  # save files in dirs with dates specified
                if not os.path.exists(fdir_out_dt):
                    os.makedirs(fdir_out_dt)
...
def run(
    ...
    for product in products:
        ...
        if "RGB" in product.upper():
            p_fnames = [
                earthdata.download_worldview_image(
                    date=date,
                    extent=extent,
                    fdir_out=fdir_out,
        ...
        # MODIS BRDF product
            ...
            for filename_tag in filename_tags_43:
                p_fnames = earthdata.download_laads_https(
                    date=date,
        ...
        # MODIS Level-1b radiances, Level-2 cloud products, surface reflectance products, and solar/viewing geoemetries
            ...
            if nrt:
                if product.upper().endswith(
                    ("VNP_CLDPROP_L2", "VJ1_CLDPROP_L2", "VJ2_CLDPROP_L2")
                ):
                ...
                msg = (
                    "Warning [sdown]: Downloading Near Real Time (NRT) products. \n"
                ...
                filename_tags_03 = earthdata.get_satfile_tag(
                    date=date,
                    start_dt_hhmm=start_dt_hhmm,
                    end_dt_hhmm=end_dt_hhmm,
                    lon=lon_2d,
                ...
                for filename_tag in filename_tags_03:
                    p_fnames = earthdata.download_lance_https(
                        date=date,
                        dataset_tag=product_info["dataset_tag"],
                        filename_tag=filename_tag,
                        day_interval=1,
                ...
                if product.upper().endswith(
                    "29"
                ):  # NRT product lives on LANCE server, standard product on NSIDC servver
                    p_fnames = earthdata.download_nsidc_https(
                        date=date,
                        extent=extent,
        ...
        if verbose:
            satlogger.info(
                "Message [sdown]: Downloaded {} files for {} ({} onboard {})".format(
                    product_download_counter,
                    product_info["dataset_tag"],
                    product_info["instrument"],
                    product_info["satellite"],
                )
            )
    ...
    if download_counter > 0:
        ...
        satlogger.info(
            "Message [sdown]: No satellite files were found for the given parameters"
        )
...
def main():
    ...
    parser = ArgumentParser(
        prog="sdown", formatter_class=RawTextHelpFormatter, description=_description_
    )
    parser.add_argument(
        "--fdir",
        type=str,
    ...
    if args.run_demo:
        ...
        # initialize logger
        satlogger = Ear3tLogger(
            _prog_, verbose=args.verbose, log_dir=er3t.common.fdir_logs
        )
