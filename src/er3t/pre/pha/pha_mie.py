...
class pha_mie_wc:
    ...
    def run(self, fname, wvl0, angles):
        Na = angles.size
...
def read_mie_shd(fname, Npmom_max=1000):
    ...
    for iline, line in enumerate(lines[6:]):
        if "Reff  Ext  Alb  Nrank" in line:
            ...
            idata_ = iline + 1 + 6
            line_pmom = ""
            while (idata_ < len(lines)) and (
                "Reff  Ext  Alb  Nrank" not in lines[idata_]
            ):
                line_pmom += lines[idata_]
                idata_ += 1

            pmom[idata, :Npmom] = np.array(
                [float(item) for item in line_pmom.split()], dtype=np.float32
            )[:Npmom]
...
class pha_mie_wc_shd:
    ...
    def run(
        ...
        for ireff in range(Nreff):
            ...
            pha0 = er3t.pre.pha.legendre2phase(
                pmom[ireff, logic_pmom],
                angle=ang0,
                lrt=False,
                normalize=True,
                deltascaling=False,
...
def legendre2phase(
    poly_coef, angle=None, deltascaling=True, normalize=False, step=0.01, lrt=False
):
...
def pmom2phase(pmom, mu_target):
    ...
    for order, omega_l in enumerate(pmom):
        pl = eval_legendre(order, mu_target)
        phase_rec += (2 * order + 1) * omega_l * pl
    return phase_rec
...
class pha_mie_wc_pmom:
    ...
    def run(self, fname, wvl0, angles):
        ...
        for ireff in range(Nreff):
            ...
            pha0 = er3t.pre.pha.legendre2phase(
                pmom0, angle=angles, lrt="True", deltascaling=True, normalize=False
            )
            pha[:, ireff] = pha0
