...
def cal_ocean_brdf(
    ...
    refrac_r = np.interp(
        wvl, refractive_index_water["wvl"], refractive_index_water["real"]
    )
    refrac_i = np.interp(
        wvl, refractive_index_water["wvl"], refractive_index_water["imaginary"]
    )
    ...
    if whitecaps:
        ...
        ref_whitecap = np.interp(
            wvl, reflectance_whitecaps["wvl"], reflectance_whitecaps["ref"]
        )
    ...
    b = 0.30 * pigment**0.62
    bbt = 0.002 + 0.02 * (
        0.5 - 0.25 * np.log10(np.maximum(pigment, 1.0e-4))
    ) * 0.550 / np.maximum(wvl, 1.0e-12)
    ...
    for _ in range(100):
        ...
        if np.all(
            np.abs((ref_underwater - ref0) / np.maximum(ref_underwater, 1.0e-30))
            < 1.0e-4
        ):
            break
        ref0 = ref_underwater
