...
class aer_gen:
    ...
    def __init__(
        ...
        if (
            ...
            sys.exit(
                f"Error   [aer_gen]: Please check if '{self.fname}' exists or provide 'fname_h4' to proceed."
            )
    ...
    def load(self, fname):
        with open(fname, "rb") as f:
            ...
            if hasattr(obj, "lev") and hasattr(obj, "lay"):
                if self.verbose:
                    er3t.common.logger.info(f"Message [cld_gen]: loading {fname} ...")
                self.fname = obj.fname
                self.lay = obj.lay
                self.lev = obj.lev
            else:
                sys.exit(
                    f"Error   [cld_gen]: {fname} is not the correct 'pickle' file to load."
                )

    def run(self, fname_h4):
        if self.verbose:
            er3t.common.logger.info(f"Message [cld_gen]: Processing {fname_h4} ...")
        ...
        with open(fname, "wb") as f:
            if self.verbose:
                er3t.common.logger.info(
                    f"Message [cld_gen]: saving object into {fname} ..."
                )
            pickle.dump(self, f)
    ...
    def pre_gen(self, earth_radius=6378.0, cloud_thickness=1.0):
        ...
        cot_all[logic] = (
            cot[logic] * cot0.attributes()["scale_factor"]
            + cot0.attributes()["add_offset"]
        )
        cer_all[logic] = (
            cer[logic] * cer0.attributes()["scale_factor"]
            + cer0.attributes()["add_offset"]
        )
        cth_all[logic] = (
            cth[logic] * cth0.attributes()["scale_factor"]
            + cth0.attributes()["add_offset"]
        )

        logic_pcl = (
            np.logical_not(logic)
            & (cot_pcl >= cot_pcl_range[0])
            & (cot_pcl <= cot_pcl_range[1])
            & (cer_pcl >= cer_pcl_range[0])
            & (cer_pcl <= cer_pcl_range[1])
            & (cth >= cth_range[0])
            & (cth <= cth_range[1])
        )
        cot_all[logic_pcl] = (
            cot_pcl[logic_pcl] * cot_pcl0.attributes()["scale_factor"]
            + cot_pcl0.attributes()["add_offset"]
        )
        cer_all[logic_pcl] = (
            cer_pcl[logic_pcl] * cer_pcl0.attributes()["scale_factor"]
            + cer_pcl0.attributes()["add_offset"]
        )
        cth_all[logic_pcl] = (
            cth[logic_pcl] * cth0.attributes()["scale_factor"]
            + cth0.attributes()["add_offset"]
        )
        cth_all /= 1000.0
    ...
    def downgrade(self, coarsing):
        ...
        if (self.Nx % dnx != 0) or (self.Ny % dny != 0) or (self.Nz % dnz != 0):
            sys.exit(
                "Error   [cld_gen]: the original dimension {} is not divisible with {}, please check input (dnx, dny, dnz).".format(
                    str(self.lay["Temperature"]["data"].shape), str(coarsing)
                )
            )
        else:
            ...
            if self.verbose:
                er3t.common.logger.info(
                    "Message [cld_gen]: Downgrading data from dimension {} to {} ...".format(
            ...
            self.lay["altitude"]["data"] = downscale(
                self.lay["altitude"]["data"], (self.Nz // dnz,)
            )
    ...
    def post_gen(self):
        ...
        if any([i > 0.001 for i in diff]):
            ...
            alt = np.append(
                ...
                self.lay["altitude"]["data"][-1] + dz0 / 2.0,
            )
