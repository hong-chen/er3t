...
class abs_16g:
    ...
    def cal_factor(self):
        ...
        atm_h2o_mix_log[atm_h2o_mix_log < -1.2206e01] = -1.2206e01
        atm_h2o_mix_log[atm_h2o_mix_log > -3.2061e00] = -3.2061e00
        ...
        # pressure 0:
        fac_vTp[:, 0, 0, 0] = fvd * (1.0 - ftd) * fpd  # fac000
        fac_vTp[:, 1, 0, 0] = (1.0 - fvd) * (1.0 - ftd) * fpd  # fac100
        fac_vTp[:, 0, 1, 0] = fvd * ftd * fpd  # fac010
        fac_vTp[:, 1, 1, 0] = (1.0 - fvd) * ftd * fpd  # fac110
        # pressure 1:
        fac_vTp[:, 0, 0, 1] = fvd * (1.0 - ftu) * fpu  # fac001
        fac_vTp[:, 1, 0, 1] = (1.0 - fvd) * (1.0 - ftu) * fpu  # fac101
        fac_vTp[:, 0, 1, 1] = fvd * ftu * fpu  # fac011
        fac_vTp[:, 1, 1, 1] = (1.0 - fvd) * ftu * fpu  # fac111
    ...
    def prep_absorption(self, wvl_join=980):
        ...
        if self.wvl < 300.0:
            ...
            (620.0 <= self.wvl < 640.0)
            | (680.0 <= self.wvl < 700.0)
            | (750.0 <= self.wvl < 760.0)
            | (770.0 <= self.wvl < 780.0)
        ):
            abso0 = copy.deepcopy(abso_ref)
            ...
            (1420.0 <= self.wvl < 1450.0)
            | (1560.0 <= self.wvl < 1630.0)
    ...
    def get_coefficient(self, f_h5):
        ...
        for ia, key in enumerate(sorted(self.abso.keys())):
            ...
            group_s = "%s/solar_taug.%d" % (
                abso_dict["group_s"]["data"],
            ...
            if abso_dict["factor"]["data"].ndim == 3:
                for iz in range(self.Nz):
                    vname_pdtd = "/%s/pressure.%d/temperature.%d/%s.%d" % (
                        ...
                        abso_dict["jpd"]["data"][iz] + 1,
                        abso_dict["jtd"]["data"][iz] + 1,
                        abso_dict["absorber"]["data"][1],
                        abso_dict["wvl"]["data"],
                    )
...
class abs_16g_txt:
    ...
    def __init__(
        self, wavelength=None, fname=None, atm_obj=None, overwrite=False, verbose=False
    ):
        self.verbose = verbose
        self.wvl = wavelength
    ...
    def prep_absorption(self, wvl_join=980):
        ...
        if self.wvl < 300.0:
            sys.exit(
                "Error   [abs_16g_txt]: Wavelength too short - no absorption data available."
            )
            ...
            sys.exit(
                "Error   [abs_16g_txt]: Wavelength too large - no absorption data available."
            )
    ...
    def get_coefficient(self):
        ...
        for ia, key in enumerate(sorted(self.abso.keys())):
            ...
            fname_s = "%s/solar_taug.%d" % (
                abso_dict["fdir_s"]["data"],
                np.round(abso_dict["wvl"]["data"]),
            )
            ...
            try:
                with open(fname_s) as f:
                    v1, v2, dv, npts, sol_min, sol_max = np.fromstring(
                        f.readline(), sep=" ", dtype=np.float64
                    )
...
class abs_oco:
    ...
    def __init__(
        self, fname=None, fname_idl=None, atm_obj=None, overwrite=False, verbose=False
    ):
        self.verbose = verbose
    ...
    def run(self, atm_obj):
        if self.fname_idl is None:
            sys.exit(
                "Error   [abs_oco]: Please provide 'fname_idl' (IDL gas absorption file) to proceed."
            )
    ...
    def get_coefficient(self, atm_obj, wvl_threshold=1.0):
        ...
        self.coef = {
            ...
            "abso_coef": {
                "name": "Absorption Coefficient (Nwl, Nz, Ng)",
                "data": np.swapaxes(np.transpose(abso_coef), 0, 1),
            },
            "slit_func": {
                "name": "Slit Function (Nwl, Nz, Ng)",
                "data": np.swapaxes(np.transpose(slit_func), 0, 1),
            },
    ...
    def get_coefficient(self, atm_obj, wvl_threshold=1.0):
        ...
        self.coef = {
            ...
            "abso_coef": {
                ...
                "data": np.transpose(abso_coef),
            },
            "slit_func": {
                "name": "Slit Function (Nz, Ng)",
                "data": np.transpose(slit_func),
            },
            "solar": {"name": "Solar Factor (Ng)", "data": solar},
            "weight": {"name": "Weight (Ng)", "data": weight},
        }
...
class abs_rrtmg_sw:
    ...
    def get_coef(self, iband, g_mode, atm_obj):
        ...
        #   <key_gas_upp>
        # ╭────────────────────────────────────────────────────────────────────────────╮#
        key_gas_low_bytes = f0.variables["KeySpeciesNamesLowerAtmos"][:][
            :, iband, :
        ].data
        key_gas_upp_bytes = f0.variables["KeySpeciesNamesUpperAtmos"][:][
            :, iband, :
        ].data
        ...
        for i in range(Nkey):
            key_gas_low_name0 = (
                "".join([j.decode("utf-8") for j in key_gas_low_bytes[i, :]])
                .strip()
                .lower()
            )
            key_gas_upp_name0 = (
                "".join([j.decode("utf-8") for j in key_gas_upp_bytes[i, :]])
                .strip()
                .lower()
            )
        ...
        # ╭──────────────────────────────────────────────────────────────╮#
        # coef
        coef_low = f0.variables["AbsorptionCoefficientsLowerAtmos"][:][
            g_mode, iband, :, :Ng, :, :
        ]
        coef_upp = f0.variables["AbsorptionCoefficientsUpperAtmos"][:][
            g_mode, iband, :, :Ng, :, :
        ]
        coef_key_low = f0.variables["KeySpeciesAbsorptionCoefficientsLowerAtmos"][:][
            g_mode, iband, :Ng, :, :, :
        ]
        coef_key_upp = f0.variables["KeySpeciesAbsorptionCoefficientsUpperAtmos"][:][
            g_mode, iband, :Ng, :, :, :
        ]
        coef_h2o_fore_low = f0.variables["H2OForeignAbsorptionCoefficientsLowerAtmos"][
            :
        ][g_mode, iband, :Ng, :]
        coef_h2o_fore_upp = f0.variables["H2OForeignAbsorptionCoefficientsUpperAtmos"][
            :
        ][g_mode, iband, :Ng, :]
        coef_h2o_self = f0.variables["H2OSelfAbsorptionCoefficients"][:][
            g_mode, iband, :Ng, :
        ]
...
def export_solar_16g():
    ...
    for wvl in wvls:
        ...
        toa = (
            np.sum(abs_obj.coef["solar"]["data"] * abs_obj.coef["weight"]["data"])
            * 1000.0
        )
