...
class cld_les:
    ...
    def __init__(
        ...
        if (
            ...
            msg = (
                "\nError [cld_les]: Please check if <%s> exists or provide <fname_nc> to proceed."
                % self.fname
            )
            raise OSError(msg)
            ...
            if file_correct:
                if self.verbose:
                    er3t.common.logger.info(
                        "Message [cld_les]: Loading <%s> ..." % fname
                    )
                ...
                msg = (
                    "\nError [cld_les]: <%s> is not the correct pickle file to load."
                    % fname
                )
                raise OSError(msg)
    ...
    def run(self, fname_nc):
        if self.verbose:
            er3t.common.logger.info("Message [cld_les]: Processing <%s> ..." % fname_nc)
    ...
    def dump(self, fname):
        ...
        with open(fname, "wb") as f:
            if self.verbose:
                er3t.common.logger.info(
                    "Message [cld_les]: Saving object into <%s> ..." % fname
                )
            pickle.dump(self, f)
    ...
    def downscale(self, coarsen):
        ...
        if (self.Nx % dnx != 0) or (self.Ny % dny != 0) or (self.Nz % dnz != 0):
            msg = (
                "\nError [cld_les]: The original dimension %s is not divisible with %s, please check input (dnx, dny, dnz, dnt)."
            ...
            if self.verbose:
                er3t.common.logger.info(
                    "Message [cld_les]: Downscaling data from dimension %s to %s ..."
