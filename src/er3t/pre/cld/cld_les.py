...
class cld_les:
    ...
    def __init__(
        ...
        if (
            ...
            msg = f"\nError [cld_les]: Please check if <{self.fname}> exists or provide <fname_nc> to proceed."
            raise OSError(msg)
    ...
    def load(self, fname):
        with open(fname, "rb") as f:
            ...
            if file_correct:
                if self.verbose:
                    er3t.common.logger.info(f"Message [cld_les]: Loading <{fname}> ...")
                self.fname = obj.fname
                self.lay = obj.lay
                self.lev = obj.lev
            else:
                msg = f"\nError [cld_les]: <{fname}> is not the correct pickle file to load."
                raise OSError(msg)

    def run(self, fname_nc):
        if self.verbose:
            er3t.common.logger.info(f"Message [cld_les]: Processing <{fname_nc}> ...")

        # pre process
        self.pre(fname_nc)

        # post process
        self.post(self.altitude)
        ...
        with open(fname, "wb") as f:
            if self.verbose:
                er3t.common.logger.info(
                    f"Message [cld_les]: Saving object into <{fname}> ..."
                )
            pickle.dump(self, f)
    ...
    def pre(self, fname_nc, q_factor=2.0, index_t=0):
        ...
        if self.coarsen[2] > 1:
            index_e = min(
                self.coarsen[2] * ((Nz0 + index_e) // self.coarsen[2] + 1), Nz0
            )
        ...
        self.lay["extinction"] = {
            "data": ext_3d,
            "name": "Extinction coefficients (3D)",
            "units": "m^-1",
        }
        self.lay["cer"] = {
            "data": cer_3d,
            "name": "Cloud effective radius (3D)",
            "units": "mm",
        }
        self.lay["lwc"] = {
            "data": lwc_3d,
            "name": "Liquid water content (3D)",
            "units": "kg m^-3",
        }
        # ╰──────────────────────────────────────────────────────────────╯#

    def post(self, altitude):
        """
        ...
        # in km
        zs_ = max(
            self.lay["altitude"]["data"][0] - self.lay["thickness"]["data"][0] / 2.0,
            0.0,
        )
        zm_ = self.lay["altitude"]["data"][1:] - self.lay["thickness"]["data"][1:] / 2.0
        ...
        self.lev["cot_2d"] = {
            "data": np.sum(cot_3d, axis=-1),
            "name": "Cloud optical thickness (2D)",
        }
        # ╰────────────────────────────────────────────────────────────────────────────╯#
    ...
    def downscale(self, coarsen):
        ...
        if (self.Nx % dnx != 0) or (self.Ny % dny != 0) or (self.Nz % dnz != 0):
            msg = "\nError [cld_les]: The original dimension {} is not divisible with {}, please check input (dnx, dny, dnz, dnt).".format(
            ...
            if self.verbose:
                er3t.common.logger.info(
                    "Message [cld_les]: Downscaling data from dimension {} to {} ...".format(
                        str(self.lay["temperature"]["data"].shape), str(new_shape)
                    )
                )

            self.lay["x"]["data"] = downscale(
                self.lay["x"]["data"], (self.Nx // dnx,), operation="mean"
            )
            self.lay["y"]["data"] = downscale(
                self.lay["y"]["data"], (self.Ny // dny,), operation="mean"
            )
            self.lay["altitude"]["data"] = downscale(
                self.lay["altitude"]["data"], (self.Nz // dnz,), operation="mean"
            )
            self.lay["pressure"]["data"] = downscale(
                self.lay["pressure"]["data"], (self.Nz // dnz,), operation="mean"
            )
            self.lay["thickness"]["data"] = downscale(
                self.lay["thickness"]["data"], (self.Nz // dnz,), operation="sum"
            )
                ...
                if isinstance(self.lay[key]["data"], np.ndarray):
                    if self.lay[key]["data"].ndim >= 3:
                        self.lay[key]["data"] = downscale(
                            self.lay[key]["data"], new_shape, operation="mean"
                        )
