...
class cld_gen_hem:
    ...
    def __init__(
        ...
        # data into pickle file
            ...
            (self.fname is not None) and (os.path.exists(self.fname)) and (overwrite)
        ) or ((self.fname is not None) and (not os.path.exists(self.fname))):
            self.run()
    ...
    def run(self):
        ...
        zlay_end = (
            min(
                [self.altitude[-1], max(self.radii) / self.w2h_ratio + self.altitude[0]]
            )
            + self.dz / 2.0 * 1.01
        )
        ...
        self.x_3d, self.y_3d, self.z_3d = np.meshgrid(
        ...
        while (self.cloud_frac < self.cloud_frac_tgt) and (self.can_add_more):
            self.add_hem_cloud(
                np.random.choice(self.radii, p=self.weights),
                min_dist=self.min_dist,
                w2h_ratio=self.w2h_ratio,
                limit=1,
            )

        self.lev = {}
        alt_lev = np.append(
            self.altitude - self.dz / 2.0, self.altitude[-1] + self.dz / 2.0
        )
    ...
    def add_hem_cloud(self, radius, min_dist=0, w2h_ratio=1.0, limit=1):
        ...
        if not self.overlap:
            ...
            for cloud0 in self.clouds:
                logic_no = (
                    (self.x_2d - cloud0["x"]) ** 2 + (self.y_2d - cloud0["y"]) ** 2
                ) <= (cloud0["radius"] + radius + min_dist) ** 2
        ...
        if N_avail > limit:
            ...
            logic_cloud0 = (
                (self.x_3d[index_x_s:index_x_e, index_y_s:index_y_e, :] - loc_x) ** 2
                + (self.y_3d[index_x_s:index_x_e, index_y_s:index_y_e, :] - loc_y) ** 2
                + (self.z_3d[index_x_s:index_x_e, index_y_s:index_y_e, :] * w2h_ratio)
                ** 2
            ) <= radius**2
    ...
    def cal_cld_opt_prop(self, ext0=0.03, cer0=12.0, cot_scale=1.0):
        ...
        self.lev["cth_2d"] = {
            "data": np.sum(data, axis=-1) + self.lev["altitude"]["data"][0],
            "name": "Cloud top height",
            "units": "km",
        }

    def update_clouds(self, w2h_ratio=2.0, cot_scale=1.0, coarsen=[1, 1, 1]):
        """
    ...
    def downscale(self, coarsen):
        ...
        if (self.Nx % dnx != 0) or (self.Ny % dny != 0) or (self.Nz % dnz != 0):
            ...
            self.lay["x"]["data"] = downscale(
                self.lay["x"]["data"], (new_shape[0],), operation="mean"
            )
            self.lay["y"]["data"] = downscale(
                self.lay["y"]["data"], (new_shape[1],), operation="mean"
            )
            self.lay["z"]["data"] = downscale(
                self.lay["z"]["data"], (new_shape[2],), operation="mean"
            )

            self.lay["altitude"]["data"] = downscale(
                self.lay["altitude"]["data"], (new_shape[2],), operation="mean"
            )
            self.lay["thickness"]["data"] = downscale(
                self.lay["thickness"]["data"], (new_shape[2],), operation="sum"
            )
                ...
                if isinstance(self.lay[key]["data"], np.ndarray):
                    if self.lay[key]["data"].ndim == len(coarsen):
                        if key in ["extinction", "cot"]:
                            ...
                            operation = "max"
                        self.lay[key]["data"] = downscale(
                            self.lay[key]["data"], new_shape, operation=operation
                        )
            # =============================================================================
            ...
            # =============================================================================
            alt_lev = np.append(
                self.lay["altitude"]["data"] - self.lay["dz"]["data"] / 2.0,
                self.lay["altitude"]["data"][-1] + self.lay["dz"]["data"] / 2.0,
            )
    ...
    def run(self, cot0, cer0, atm_obj=None):
        ...
        if atm_obj is None:
            ...
            t_1d = np.interp(
                ...
                atm_obj.lay["altitude"]["data"],
                atm_obj.lay["temperature"]["data"],
            )
        # ╰────────────────────────────────────────────────────────────────────────────╯#
...
class cld_gen_cop:
    ...
    def __init__(
        ...
        if (
            ...
            (cot is not None)
            and (self.fname is not None)
            and (not os.path.exists(self.fname))
        ):
    ...
    def process(
        ...
        logic_clear = np.isnan(cloud_optical_thickness) | (
            cloud_optical_thickness <= 0.0
        )
        if not isinstance(cloud_effective_radius, np.ndarray):
            cloud_effective_radius = np.full(
                cloud_optical_thickness.shape, cloud_effective_radius
            )
        ...
        if not isinstance(cloud_geometrical_thickness, np.ndarray):
            cloud_geometrical_thickness = np.full(
                cloud_optical_thickness.shape, cloud_geometrical_thickness
            )
    ...
    def get_extra_cloud_property(self, cer_mode="top", fill_clear=0.0, overwrite=False):
        ...
        if (
            ("cld_msk" not in self.lay.keys())
            or ("cld_msk_2d" not in self.lev.keys())
            or (overwrite)
        ):
        ...
        if ("cot_2d" not in self.lev.keys()) or (overwrite):
            cot_2d = np.nansum(
                self.lay["extinction"]["data"]
                * self.lay["thickness"]["data"][0]
                * 1000.0,
                axis=-1,
            )
        ...
        # ╭────────────────────────────────────────────────────────────────────────────╮#
        if (
            ("cer_2d" not in self.lev.keys())
            or ("cth_2d" not in self.lev.keys())
            or ("cbh_2d" not in self.lev.keys())
            or (overwrite)
        ):
            ...
            cer_2d = np.zeros((Nx, Ny), dtype=er3t.common.f_dtype)
            cer_2d[...] = fill_clear

            cth_2d = np.zeros((Nx, Ny), dtype=er3t.common.f_dtype)
            cth_2d[...] = fill_clear
            cbh_2d = np.zeros((Nx, Ny), dtype=er3t.common.f_dtype)
            cbh_2d[...] = fill_clear
