...
def modify_h2o(
    ...
    *,
    ...
    """Build an atmosphere with humidity and temperature from housekeeping data.

    The project-specific atmosphere and housekeeping readers are injected rather
    than imported implicitly. This keeps the utility reusable and avoids a
    circular dependency on the atmospheric preprocessing module.
    """

    atm_atmmod0 = atmosphere_factory(levels=levels, fname_atmmod=fname_atmmod)

    atm0 = atm_atmmod0.lev.copy()
    alt_atm = atm0["altitude"]["data"]

    hsk0 = housekeeping_reader(date)
    ...
    wvnd_hsk = cal_num_den_from_rel_hum(
        hsk0.data["Relative_Humidity"][logic_hsk],
        hsk0.data["Static_Air_Temp"][logic_hsk],
    )
    ...
    f_interp = interpolate.interp1d(
        alt_hsk, wvnd_hsk, bounds_error=False, fill_value="extrapolate", kind="linear"
    )
    atm0["h2o"]["data"][logic_alt] = f_interp(alt_atm[logic_alt])
    scale_factor = (
        atm0["h2o"]["data"][indices_alt[-1]] / atm0["h2o"]["data"][indices_alt[-1] + 1]
    )
    atm0["h2o"]["data"][indices_alt[-1] + 1 :] *= scale_factor

    f_interp = interpolate.interp1d(
        alt_hsk, temp_hsk, bounds_error=False, fill_value="extrapolate", kind="linear"
    )
    atm0["temperature"]["data"][logic_alt] = f_interp(alt_atm[logic_alt])
    scale_factor = (
        atm0["temperature"]["data"][indices_alt[-1]]
        / atm0["temperature"]["data"][indices_alt[-1] + 1]
    )
