...
from er3t.core.numerics import cal_r_twostream
...
def gen_cloud_1d(cld_cfg):
    ...
    with open(cld_cfg["cloud_file"], "w") as f:
        f.write(
            "# Altitude[km]    Liquid Water Content [g/m3]    Cloud Effective Radius [um]\n"
        )
...
def cal_radiative_property(
    f_up, f_down, topN=-1, bottomN=0, scaleN=1.0, tag="albedo-top"
):
    """
    ...
    if tag.lower() == "transmittance":
        ...
        reflectance = (
            (f_up[0, topN, :] - f_up[0, bottomN, :]) / f_down[0, topN, :] * scaleN
        )
        return reflectance
...
def gen_bispectral_lookup_table(
    ...
    cloud_optical_thickness_all=np.arange(
        0.0, 50.1, 2.0
    ),  # cloud optical thickness array (numpy.ndarray)
    cloud_effective_radius_all=np.arange(
        4.0, 25.1, 1.0
    ),  # cloud effective radius (numpy.ndarray)
    ...
    for cot in cloud_optical_thickness_all:
        for cer in cloud_effective_radius_all:
            ...
            input_file_x = (
                "%s/lrt_inpfile_wvl-%4.4dnm_cot-%04.1f_cer-%04.1f_sza-%04.1f_alb-%04.2f.txt"
                % (fdir_tmp, wvl_x, cot, cer, sza, alb_x)
            )
            output_file_x = (
                "%s/lrt_outfile_wvl-%4.4dnm_cot-%04.1f_cer-%04.1f_sza-%04.1f_alb-%04.2f.txt"
                % (fdir_tmp, wvl_x, cot, cer, sza, alb_x)
            )

            input_file_y = (
                "%s/lrt_inpfile_wvl-%4.4dnm_cot-%04.1f_cer-%04.1f_sza-%04.1f_alb-%04.1f.txt"
                % (fdir_tmp, wvl_y, cot, cer, sza, alb_y)
            )
            output_file_y = (
                "%s/lrt_outfile_wvl-%4.4dnm_cot-%04.1f_cer-%04.1f_sza-%04.1f_alb-%04.1f.txt"
                % (fdir_tmp, wvl_y, cot, cer, sza, alb_y)
            )
    ...
    if prop_tag.lower() in ["radiance", "rad", "reflectance", "ref"]:
        ...
        prop_x = data_x.rad[0, -1, :].reshape(
            (cloud_optical_thickness_all.size, cloud_effective_radius_all.size)
        )
        prop_y = data_y.rad[0, -1, :].reshape(
            (cloud_optical_thickness_all.size, cloud_effective_radius_all.size)
        )
        ...
        # ╭────────────────────────────────────────────────╮#
        prop_x = cal_radiative_property(
            data_x.f_up, data_x.f_down, tag=prop_tag
        ).reshape((cloud_optical_thickness_all.size, cloud_effective_radius_all.size))
        prop_y = cal_radiative_property(
            data_y.f_up, data_y.f_down, tag=prop_tag
        ).reshape((cloud_optical_thickness_all.size, cloud_effective_radius_all.size))
        # ╰────────────────────────────────────────────────╯#
...
def retrieve(prop1, prop2, cld_tau, cld_ref, prop1_data, prop2_data):
    ...
    points_x1 = prop1_data[0, :]
    points_y1 = prop2_data[0, :]
    points_x2 = prop1_data[:, -1]
    points_y2 = prop2_data[:, -1]
    points_x3 = prop1_data[-1, :][::-1]
    points_y3 = prop2_data[-1, :][::-1]
    points_x4 = prop1_data[:, 0][::-1]
    points_y4 = prop2_data[:, 0][::-1]
    ...
    if grid_path_full.contains_point((prop1, prop2)):
        ...
        while (not grid_path.contains_point((prop1, prop2))) and (index_x <= Nx - 3):
            index_x += 1
            points_x = np.append(
                prop1_data[index_x, :], prop1_data[index_x + 1, :][::-1]
            )
            points_y = np.append(
                prop2_data[index_x, :], prop2_data[index_x + 1, :][::-1]
            )
            points_xy = np.transpose(np.vstack((points_x, points_y)))
            grid_path = mpl_path.Path(points_xy, closed=True)
        ...
        while (not grid_path.contains_point((prop1, prop2))) and (index_y <= Ny - 3):
            index_y += 1
            points_x = np.append(
                prop1_data[:, index_y], prop1_data[:, index_y + 1][::-1]
            )
            points_y = np.append(
                prop2_data[:, index_y], prop2_data[:, index_y + 1][::-1]
            )
            points_xy = np.transpose(np.vstack((points_x, points_y)))
            grid_path = mpl_path.Path(points_xy, closed=True)
        ...
        points[0, :] = np.array(
            [prop1_data[index_x, index_y], prop2_data[index_x, index_y]]
        )
        points_tau[0] = cld_tau[index_x]
        points_ref[0] = cld_ref[index_y]
        points[1, :] = np.array(
            [prop1_data[index_x, index_y + 1], prop2_data[index_x, index_y + 1]]
        )
        points_tau[1] = cld_tau[index_x]
        points_ref[1] = cld_ref[index_y + 1]
        points[2, :] = np.array(
            [prop1_data[index_x + 1, index_y], prop2_data[index_x + 1, index_y]]
        )
        points_tau[2] = cld_tau[index_x + 1]
        points_ref[2] = cld_ref[index_y]
        points[3, :] = np.array(
            [prop1_data[index_x + 1, index_y + 1], prop2_data[index_x + 1, index_y + 1]]
        )
...
class func_ref_vs_cot:
    def __init__(
        ...
        self.ref_2s = cal_r_twostream(cot, a=self.alb0, mu=self.mu0)
    ...
    def get_inits(self):
        ...
        for cot0 in self.cot:
            ...
            inp_file_rad = (
                "%s/lrt-rad-inp_wvl-%4.4dnm_cot-%04.1f_cer-%04.1f_sza-%04.1f_alb-%04.2f.txt"
                % (self.fdir, self.wvl0, cot0, self.cer0, self.sza0, self.alb0)
            )
            out_file_rad = (
                "%s/lrt-rad-out_wvl-%4.4dnm_cot-%04.1f_cer-%04.1f_sza-%04.1f_alb-%04.2f.txt"
                % (self.fdir, self.wvl0, cot0, self.cer0, self.sza0, self.alb0)
            )
