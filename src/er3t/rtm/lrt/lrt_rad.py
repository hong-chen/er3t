    ...
    def __init__(
        ...
        if not isinstance(sensor_azimuth_angle, str):
            if isinstance(sensor_azimuth_angle, (list, np.ndarray)):
                ...
                sensor_azimuth_angle = " ".join(
                    str(vaa0) for vaa0 in sensor_azimuth_angle
                )
            else:
        ...
        if aer_cfg is not None:
            if all(aer_cfg[key] is not None for key in aer_cfg.keys()):
                ...
                if self.input_dict_extra is not None:
                    ...
                    self.input_dict_extra["aerosol_file tau"] = aer_cfg[
                        "aerosol_file_aod"
                    ]
                    self.input_dict_extra["aerosol_file ssa"] = aer_cfg[
                        "aerosol_file_ssa"
                    ]
                    self.input_dict_extra["aerosol_file gg"] = aer_cfg[
                        "aerosol_file_asy"
                    ]
        ...
        if aer_cfg is not None:
            if all(aer_cfg[key] is not None for key in aer_cfg.keys()):
                ...
                if self.input_dict_extra:
                    ...
                    self.input_dict_extra["aerosol_modify tau set"] = str(
                        aer_cfg["aerosol_optical_depth"]
                    )
                    self.input_dict_extra["aerosol_modify gg set"] = str(
                        aer_cfg["asymmetry_parameter"]
                    )
                    self.input_dict_extra["aerosol_modify ssa set"] = str(
                        aer_cfg["single_scattering_albedo"]
                    )
                    ...
                    self.input_dict_extra = OD(
                        [
                            ...
                            (
                                "aerosol_modify tau set",
                                str(aer_cfg["aerosol_optical_depth"]),
                            ),
                            (
                                "aerosol_modify gg set",
                                str(aer_cfg["asymmetry_parameter"]),
                            ),
                            (
                                "aerosol_modify ssa set",
                                str(aer_cfg["single_scattering_albedo"]),
                            ),
                        ]
                    )
