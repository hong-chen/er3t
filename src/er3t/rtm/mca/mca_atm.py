...
from er3t.core.numerics import cal_mol_ext, cal_sol_fac, get_lay_index, cal_mol_ext_atm
...
class mca_atm_1d:
    ...
    def __init__(self, atm_obj=None, abs_obj=None):
        if atm_obj is None:
    ...
    def pre_mca_1d_atm(self):
        ...
        for ig in range(self.Ng):
            self.nml[ig]["Atm_zgrd0"] = {
                "data": self.atm.lev["altitude"]["data"] * 1000.0,
                "units": "m",
                "name": "Layer boundaries",
            }
            self.nml[ig]["Atm_wkd0"] = {
                "data": 1.0,
                "units": "N/A",
                "name": "Weight coefficients",
            }
            self.nml[ig]["Atm_mtprof"] = {
                "data": 0,
                "units": "N/A",
                "name": "Temperature profile flag",
            }
            self.nml[ig]["Atm_tmp1d"] = {
                "data": self.atm.lay["temperature"]["data"],
                "units": "K",
            ...
            self.nml[ig]["Atm_nkd"] = {
                "data": 1,
                "units": "N/A",
                "name": "Number of K-distribution",
            }
            self.nml[ig]["Atm_np1d"] = {
                "data": 1,
                "units": "N/A",
                "name": "Number of 1D atmospheric constituents",
            }
            ...
            atm_sca = cal_mol_ext_atm(self.abs.wvl * 0.001, self.atm) / (
                self.atm.lay["thickness"]["data"] * 1000.0
            )

            # Absorption coefficient
            atm_abs = self.abs.coef["abso_coef"]["data"][:, ig] / (
                self.atm.lay["thickness"]["data"] * 1000.0
            )
            self.nml[ig]["Atm_abs1d(1:, 1)"] = {
                "data": atm_abs,
                "units": "/m",
    ...
    def add_mca_1d_atm(
        self, ext1d=None, omg1d=None, apf1d=None, z_bottom=None, z_top=None
    ):
...
class mca_atm_3d:
    ...
    def __init__(
        ...
        if (
            ...
            msg = (
                "Error [mca_atm_3d]: Incorrect number of cloud layers (%d) vs layer thicknesses (%d)."
                % (
                    self.cld.lay["altitude"]["data"].size,
                    self.cld.lay["thickness"]["data"].size,
                )
    ...
    def pre_mca_3d_atm(self):
        ...
        lay_index = get_lay_index(
            self.cld.lay["altitude"]["data"], self.atm.lay["altitude"]["data"]
        )
        ...
        if (iz3l + nz3) > self.atm.lay["altitude"]["data"].size:
            msg = (
                "Error [mca_atm_3d]: Non-homogeneous layer top exceeds atmosphere top."
            )
            raise ValueError(msg)

        atm_tmp = np.zeros(
            (nx, ny, nz3), dtype=np.float32
        )  # deviation from layer temperature (-)
        atm_abs = np.zeros(
            (nx, ny, nz3, 1), dtype=np.float32
        )  # deviation from atmospheric absorption (-)
        atm_ext = np.zeros((nx, ny, nz3, 1), dtype=np.float32)  # extinction
        atm_omg = np.zeros(
            (nx, ny, nz3, 1), dtype=np.float32
        )  # single scattering albedo (set to 1.0)
        atm_apf = np.zeros((nx, ny, nz3, 1), dtype=np.float32)  # phase function

        for i in range(nz3):
            atm_tmp[:, :, i] = (
                self.cld.lay["temperature"]["data"][:, :, i]
                - self.atm.lay["temperature"]["data"][lay_index[i]]
            )
        ...
        if self.pha is None:
            ...
            if self.pha.data["id"]["data"].lower() == "hg":
                ...
                f_interp_ssa = interpolate.interp1d(
                    ref, ssa, bounds_error=False, fill_value="extrapolate"
                )
                ...
                if self.phase:
                    ...
                    f_interp_ind = interpolate.interp1d(
                        ref, ind, bounds_error=False, fill_value="extrapolate"
                    )
                    ...
                    f_interp_asy = interpolate.interp1d(
                        ref, asy, bounds_error=False, fill_value="extrapolate"
                    )
        ...
        self.nml["Atm_iz3l"] = {
            "data": iz3l,
            "unit": "N/A",
            "name": "layer index of first 3D layer",
        }
        ...
        self.nml["Atm_abst3d"] = {
            "data": atm_abs,
            "units": "/m",
            "name": "Absorption coefficients deviation",
        }
        self.nml["Atm_extp3d"] = {
            "data": atm_ext,
            "units": "/m",
        ...
        self.nml["Atm_np3d"] = {
            "data": 1,
            "units": "N/A",
            "name": "Number of 3D atmospheric constituents",
        }
    ...
    def add_mca_3d_atm(self, ext3d=None, omg3d=None, apf3d=None):
        ...
        atm_ext = np.concatenate(
            (self.nml["Atm_extp3d"]["data"], ext3d[..., np.newaxis]), axis=-1
        )
        atm_omg = np.concatenate(
            (self.nml["Atm_omgp3d"]["data"], omg3d[..., np.newaxis]), axis=-1
        )
        atm_apf = np.concatenate(
            (self.nml["Atm_apfp3d"]["data"], apf3d[..., np.newaxis]), axis=-1
        )
