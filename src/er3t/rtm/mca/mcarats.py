            ...
            if sca is None:
                ...
                for key in sca.nml.keys():
                    if os.path.exists(sca.nml["Sca_inpfile"]["data"]):
                        sca.nml["Sca_inpfile"]["data"] = os.path.relpath(
                            sca.nml["Sca_inpfile"]["data"], start=self.fdir
                        )
                    self.nml[ig][key] = sca.nml[key]["data"]

    def init_atm(self, atm_1ds=None, atm_3ds=None):
        atm_1ds = () if atm_1ds is None else atm_1ds
        atm_3ds = () if atm_3ds is None else atm_3ds
        for ig in range(self.Ng):
            if len(atm_1ds) == 0:
            ...
            if len(atm_3ds) > 0:
                    ...
                    for key in atm_3d.nml.keys():
                        if key not in [
                            ...
                            if os.path.exists(atm_3d.nml["Atm_inpfile"]["data"]):
                                atm_3d.nml["Atm_inpfile"]["data"] = os.path.relpath(
                                    atm_3d.nml["Atm_inpfile"]["data"], start=self.fdir
                                )
                ...
                if self.target == "radiance":
                    if "satellite" in self.sensor_type.lower():
                        if (
                            "Atm_nx" in atm_3d.nml.keys()
                            and "Atm_ny" in atm_3d.nml.keys()
                        ):
    ...
    def init_sfc(self, surface=0.03):
        for ig in range(self.Ng):
            ...
            if (
                ...
                or isinstance(surface, np.float32)
                or isinstance(surface, np.float64)
            ):
                ...
                for key in surface.nml.keys():
                    if "2d" not in key:
                        if os.path.exists(surface.nml["Sfc_inpfile"]["data"]):
                            surface.nml["Sfc_inpfile"]["data"] = os.path.relpath(
                                surface.nml["Sfc_inpfile"]["data"], start=self.fdir
                            )
...
def distribute_photon(Nphoton, weights, base_ratio=0.05):
    Ndist = weights.size
    photons_dist = np.int_(Nphoton * (1.0 - base_ratio) * weights) + np.int_(
        Nphoton * base_ratio / Ndist
    )
