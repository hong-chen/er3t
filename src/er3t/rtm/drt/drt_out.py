...
def get_shd_data_out_ori(
    ...
    if verbose:
        er3t.common.logger.info("Message [get_shd_data_out]: Reading SHDOM output ...")
    ...
    Nline_output_type = [
        i for i in range(len(headers)) if ("OUTPUT_TYPE=" in headers[i])
    ][0]
    ...
    line_xy = [
        line
        for line in headers[Nline_output_type:]
...
def get_shd_data_out(
    ...
    if binary:
        ...
        if mode in ["F5", "H3", "S2", "J2", "M2"]:
            Ndata, Nvar = [
                int(num_s.strip()) for num_s in headers[-1].replace("!", "").split(",")
            ]
            shape = (Ndata, Nvar)
...
class shd_out_ng:
    ...
    def __init__(
        ...
        if (fname is not None) and (os.path.exists(fname)) and (not overwrite):
            ...
            ((shd_obj is not None) and (abs_obj is not None))
...
class shd_out_raw:
    ...
    def read_txt(self):
        if self.verbose:
            er3t.common.logger.info(
                "Message [get_shd_data_out]: Reading SHDOM output ..."
            )
            er3t.common.logger.info(
                "╭────────────────────────────────────────────────────────────────────────────╮"
            )
        ...
        if binary:
            ...
            if mode in ["F5", "H3", "S2", "J2", "M2"]:
                Ndata, Nvar = [
                    int(num_s.strip())
                    for num_s in headers[-1].replace("!", "").split(",")
                ]
                shape = (Ndata, Nvar)
