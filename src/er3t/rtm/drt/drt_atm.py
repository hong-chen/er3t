...
class drt_prp_1d:
    ...
    def __init__(
        ...
        if (
            ...
            msg = (
                "Error [drt_prp_1d]: Incorrect number of cloud layers (%d) vs layer thicknesses (%d)."
                % (
                    self.cld.lay["altitude"]["data"].size,
                    self.cld.lay["thickness"]["data"].size,
                )
        ...
        if not self.overwrite:
            if (not os.path.exists(fname)) and (not force):
                self.gen_drt_prp_file(
                    fname, self.abs.wvl, self.atm, self.cld, fname_ckd_1d=fname_ckd_1d
                )
    ...
    def gen_drt_prp_file(
        ...
        if fname_ckd_1d is not None:
            fname_inp = er3t.rtm.drt.gen_ext_file(
                fname.replace("prp", "ext"), cld0, fname_ckd_1d=fname_ckd_1d
            )
        else:
