...
class drt_ckd_1d:
    ...
    def __init__(self,
        ...
        if atm_obj is None:
            msg = 'Error [drt_ckd_1d]: please provide an atm object for <atm_obj>.'
            raise OSError(msg)
        else:
            self.atm = atm_obj

        if abs_obj is None:
            msg = 'Error [drt_ckd_1d]: please provide an abs object for <abs_obj>.'
            raise OSError(msg)
        else:
            self.abs = abs_obj
        ...
        if fname is None:
            fname = 'disort-ckd.txt'

        if not self.overwrite:
            if (not os.path.exists(fname)) and (not force):
                self.gen_drt_ckd_file(fname, self.atm, self.abs, alt_toa=alt_toa)
            self.nml['CKDFILE'] = {'data':fname}
        else:
            self.gen_drt_ckd_file(fname, self.atm, self.abs, alt_toa=alt_toa)


    def pre_drt_1d_atm(self):
        ...
        self.atm_sca = cal_mol_ext_atm(self.abs.wvl/1000.0, self.atm) / (self.atm.lay['thickness']['data'])


    def gen_drt_ckd_file(
            self,
            fname,
            atm0,
            abs0,
        ...
        if not self.quiet:
            print('Message [drt_ckd_1d]: Creating 1D CKDFile <%s> for DISORT...' % fname)
...
class drt_prp_1d:
    ...
    Output:
        ...
        self.gen_drt_prp_file: method to create DISORT property file of 3d atmosphere
    """


    ID = 'DISORT Atmosphere 1D'
    ...
    def __init__(self,\
        ...
        if atm_obj is None:
            msg = 'Error [drt_prp_1d]: Please provide an atm object for <atm_obj>.'
            raise OSError(msg)
        else:
            self.atm = atm_obj

        if abs_obj is None:
            msg = 'Error [drt_prp_1d]: Please provide an abs object for <abs_obj>.'
            raise OSError(msg)
        else:
            self.abs = abs_obj

        if cld_obj is None:
            msg = 'Error [drt_prp_1d]: Please provide an cld object for <cld_obj>.'
            raise OSError(msg)
        else:
            self.cld = cld_obj
        ...
        if self.cld.lay['altitude']['data'].size != self.cld.lay['thickness']['data'].size: # layer number
            msg = 'Error [drt_prp_1d]: Incorrect number of cloud layers (%d) vs layer thicknesses (%d).' % (self.cld.lay['altitude']['data'].size, self.cld.lay['thickness']['data'].size)
            raise ValueError(msg)

        self.pre_drt_1d_atm(alt_toa=alt_toa)

        if fname is None:
            fname = 'disort-prp.txt'

        if not self.overwrite:
            if (not os.path.exists(fname)) and (not force):
                self.gen_drt_prp_file(fname, self.abs.wvl, self.atm, self.cld, fname_ckd_1d=fname_ckd_1d)
            self.nml['PROPFILE'] = {'data':fname}
        else:
            self.gen_drt_prp_file(fname, self.abs.wvl, self.atm, self.cld, fname_ckd_1d=fname_ckd_1d)


    def pre_drt_1d_atm(self, alt_toa=30.0):
    ...
    def gen_drt_prp_file(
        ...
        # fname_mie = er3t.rtm.drt.gen_mie_file_from_nc(wavelength, wavelength)
        fname_mie = er3t.rtm.drt.gen_mie_file(wavelength, wavelength)

        if fname_ckd_1d is not None:
            fname_inp = er3t.rtm.drt.gen_ext_file(fname.replace('prp', 'ext'), cld0, fname_ckd_1d=fname_ckd_1d)
        else:
            fname_inp = er3t.rtm.drt.gen_lwc_file(fname.replace('prp', 'lwc'), cld0)

        if len(self.z_extra) > 5000:
            msg = f"Error [drt_prp_1d]: <z_extra> [length={len(self.z_extra)}] is greater than 5000-character-limit."
            raise OSError(msg)

        wavelength /= 1000.0

        Nlay = atm0.lay['pressure']['data'].size

        alt = atm0.lev['altitude']['data']
        indices_sort = np.argsort(alt)[::-1]
        alt = alt[indices_sort]
        temp = atm0.lev['temperature']['data'][indices_sort]

        alt_str  = ' '.join([f"{item:.8e}" for item in alt])
        temp_str = ' '.join([f"{item:.8e}" for item in temp])

        rayleigh_coef = f"{0.00332:.8e}"
        molecular_abs = ' '.join(["0.0" for item in np.repeat(0.0, Nlay)])

        optical_data = ' '.join(["0.0 1.0" for i in range(Nlay)])

        command = f"{put_exe} 1 {fname_mie} {Nlay} \
