...
def load_shd_inp_nml_info():
    ...
    shdom_nml_all_info = OrderedDict(
        [
            ...
            (
                "NSTOKES",
                "number of Stokes parameters (1 for I, i.e. unpolarized;\n\
                                                      3 for I, Q, U;\n\
                                                   or 4 for I, Q, U, V)",
            ),
            (
                "NX",
                "base grid size in X; NX is the number of grid points horizontally;\n\
                    for periodic boundaries there is actually an extra plane",
            ),
            (
                "NY",
                "base grid size in Y; NY is the number of grid points horizontally;\n\
                    for periodic boundaries there is actually an extra plane",
            ),
            (
                "NZ",
                "base grid size in Z; NZ is one more than the number grid cells vertically",
            ),
...
def shd_inp_file(input_fname, input_dict, indent="  ", verbose=True, comment=False):
    shdom_nml_all, shdom_nml_all_info, shdom_nml_input = shd_inp_nml(
    ...
    for nml_key in shdom_nml_all.keys():
        vars_key = [
            xx for xx in shdom_nml_input.keys() if shdom_nml_input[xx] == nml_key
        ]
