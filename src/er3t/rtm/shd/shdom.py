...
import er3t.common
from er3t.core.numerics import cal_sol_fac
...
class shdom_ng:
    ...
    def __init__(
        ...
        if (
            isinstance(self.surface, float)
            or isinstance(self.surface, np.float32)
            or isinstance(self.surface, np.float64)
        ):
            self.sfc_2d = False
    ...
    def nml_rad(
        ...
        for ig in range(self.Ng_):
            ...
            self.nml[ig]["SOLARFLUX"] = cal_sol_fac(self.date)
                ...
                if self.sensor_type == "radiometer":
                    if mu_new.size > 0:
                        ...
                        if mu_new.size <= 36:
                            view_str = "\n".join(
                                [
                                    f" {item[0]:.16e}, {item[1]:.4e},"
                                    for item in zip(mu_new, vaa_new)
                                ]
                            )
                            self.nml[ig]["OUTPARMS(1,1)"] = (
                                f"{alt0:.4e}, {dx:.8e}, {dy:.8e}, 0.0, 0.0, {mu_new.size},\n{view_str}"
                            )
                            self.nml[ig]["OUTPARMS(1,1)"] = self.nml[ig][
                                "OUTPARMS(1,1)"
                            ][:-1]  # get rid of comma (,) at the end

                        else:
                            data_sensor = OrderedDict()
                            data_sensor["vza"] = mu_new
                            data_sensor["vaa"] = vaa_new
                            fname_sensor = self.fnames_inp[ig].replace(
                                "shdom-inp", "shdom-sen"
                            )
                            self.fname_sensor = er3t.rtm.shd.gen_sen_file(
                                fname_sensor, data_sensor
                            )

                            self.nml[ig]["SENFILE"] = f"{self.fname_sensor}"
                            self.nml[ig]["OUTPARMS(1,1)"] = (
                                f"{alt0:.4f}, {dx:.8e}, {dy:.8e}, 0.0, 0.0, {mu_new.size}"
                            )
                    ...
                    self.nml[ig]["OUTPARMS(1,1)"] = (
                        f"1 {nbyte} {downscale} {self.sensor_xpos:.4f} {self.sensor_ypos:.4f} {self.sensor_altitude:.4f} {theta:.1f} {phi:.1f} {rotang:.1f} {nlines} {nsamps} {delline:.4f} {delsamp:.4f}"
                    )
                    ...
                    self.nml[ig]["OUTPARMS(1,1)"] = (
                        f"2 {nbyte} {downscale} {24.0} {20.0} {5.0} {24.0 - 5.0 * np.sin(np.deg2rad(10.0))} {20.0 + 5.0 * np.cos(np.deg2rad(10.0))} {5.0} {5.0 / 154.0} {scan1:.1f} {scan2:.1f} {delscan:.4f}"
                    )
                    ...
                    if (isinstance(data_sensor["x"], float)) or (
                        isinstance(data_sensor["x"], int)
                    ):
                        data_sensor["x"] = np.repeat(
                            data_sensor["x"], data_sensor["vza"].size
                        )
                    if (isinstance(data_sensor["y"], float)) or (
                        isinstance(data_sensor["y"], int)
                    ):
                        data_sensor["y"] = np.repeat(
                            data_sensor["y"], data_sensor["vza"].size
                        )
                    if (isinstance(data_sensor["z"], float)) or (
                        isinstance(data_sensor["z"], int)
                    ):
                        data_sensor["z"] = np.repeat(
                            data_sensor["z"], data_sensor["vza"].size
                        )
                    ...
                    self.nml[ig]["SENFILE"] = f"{self.fname_sensor}"
                    self.nml[ig]["OUTPARMS(1,1)"] = (
                        f"3 {data_sensor['vza'].size} {len(data_sensor.keys())}"
                    )
        ...
        for ig in range(self.Ng_):
            ...
            if split_acc is None:
                if (self.dx <= 0.100001) or (self.dy <= 0.100001):
                    ...
                    if (self.Nx == 1) and (self.Ny == 1):
                        self.nml[ig]["SPLITACC"] = (
                            0.00003  # follows Emde et al., 2019 (IPRT polarized radiative transfer model intercomparison project – phase A)
                        )
                    elif (self.Nx > 64) and (self.Ny > 64):
            ...
            self.nml[ig]["MAX_TOTAL_MB"] = (
                psutil.virtual_memory().total / 1024.0**2.0 / 1.2
            )
            if (self.Nx == 1) and (self.Ny == 1):
                self.nml[ig]["ADAPT_GRID_FACTOR"] = 10.0
                self.nml[ig]["CELL_TO_POINT_RATIO"] = 1.9
            else:
    ...
    def print_info(self):
        ...
        if self.target == "radiance":
            if (self.sensor_zenith_angle.size <= 36) and (
                self.sensor_type == "radiometer"
            ):
                # print first three vza, vaa pairs
                for i, vza0 in enumerate(
                    self.sensor_zenith_angle[: min([2, self.sensor_zenith_angle.size])]
                ):
