...
def anim_phase_mie(index, cer0=1.0, Nstart=5, Nend=751):
    ...
    pha_r_ = er3t.pre.pha.pha_mie_wc_shd(
        wavelength=650.0, Npmom_max=1000, angles=angles, overwrite=True
    )
    pha_g_ = er3t.pre.pha.pha_mie_wc_shd(
        wavelength=550.0, Npmom_max=1000, angles=angles, overwrite=True
    )
    pha_b_ = er3t.pre.pha.pha_mie_wc_shd(
        wavelength=450.0, Npmom_max=1000, angles=angles, overwrite=True
    )
    ...
    for Npmom_max in np.arange(Nstart, Nend, 5):
        ...
        pha_r = er3t.pre.pha.legendre2phase(
            pmom_r[logic_r], angle=angles, lrt=False, normalize=True, deltascaling=False
        )

        pmom_g = pha_g_.data["pmom"]["data"][index_cer, :Npmom_max]
        logic_g = ~np.isnan(pmom_g)
        pha_g = er3t.pre.pha.legendre2phase(
            pmom_g[logic_g], angle=angles, lrt=False, normalize=True, deltascaling=False
        )

        pmom_b = pha_b_.data["pmom"]["data"][index_cer, :Npmom_max]
        logic_b = ~np.isnan(pmom_b)
        pha_b = er3t.pre.pha.legendre2phase(
            pmom_b[logic_b], angle=angles, lrt=False, normalize=True, deltascaling=False
        )
        ...
        if plot:
            ...
            ax1.plot(
                pha_r_.data["ang"]["data"],
                pha_r_.data["pha"]["data"][:, index_cer],
                color="r",
                lw=3.0,
                alpha=0.2,
                ls="-",
            ...
            ax1.plot(
                pha_g_.data["ang"]["data"],
                pha_g_.data["pha"]["data"][:, index_cer],
                color="g",
                lw=1.8,
                alpha=0.2,
                ls="-",
            ...
            ax1.plot(
                pha_b_.data["ang"]["data"],
                pha_b_.data["pha"]["data"][:, index_cer],
                color="b",
                lw=0.9,
                alpha=0.2,
                ls="-",
            ...
            ang_full = (
                np.append(
                    pha_r_.data["ang"]["data"], 180.0 + pha_r_.data["ang"]["data"][1:]
                )
                % 360.0
            )
...
def lrt_flux_spec(
    ...
    fdir_tmp = "{}/tmp-data/{}/{}/cot-{:04.1f}_cer-{:04.1f}".format(
        ...
        name_tag,
        _metadata["Function"],
        params["cloud_optical_thickness"],
        params["cloud_effective_radius"],
    )
    ...
    cld_cfg["cloud_altitude"] = np.arange(
        params["cloud_top_height"] - params["cloud_geometric_thickness"],
        params["cloud_top_height"] + 0.01,
        0.1,
    )
...
def test_100_flux_spec(
    ...
    params = {
        ...
        "pigment": 0.01,
        "extra": None,
        "output_altitude": np.concatenate(
            (
                np.arange(0.0, 1.0, 0.05),
                np.arange(1.0, 8.0, 0.1),
                np.arange(8.0, 16.0, 0.2),
                np.arange(16.0, 30.0, 0.5),
                np.arange(30.0, 60.1, 1.0),
            )
        ),
    }
    ...
    for gas in ["all"] + gases:
        ...
        h5d_f_down = h5f.create_dataset(
            "f_down",
            data=data_lrt["f_down"],
            compression="gzip",
            compression_opts=9,
            chunks=True,
        )
        h5d_f_down = h5f.create_dataset(
            "f_down_diffuse",
            data=data_lrt["f_down_diffuse"],
            compression="gzip",
            compression_opts=9,
            chunks=True,
        )
        h5d_wvl = h5f.create_dataset(
            "wvl", data=wavelengths, compression="gzip", compression_opts=9, chunks=True
        )
        h5d_alt = h5f.create_dataset(
            "alt",
            data=params["output_altitude"],
            compression="gzip",
            compression_opts=9,
            chunks=True,
        )
...
def anim_gas_absorption(index):
    ...
    data = er3t.io.load_h5("data/data_flux_all.h5")

    atm0 = er3t.pre.atm.atm_atmmod(
        levels=data["alt"],
        fname=None,
        fname_atmmod=f"{er3t.common.fdir_data_atmmod}/afglss.dat",
        overwrite=True,
    )
    ...
    if plot:
        ...
        for i, gas in enumerate(gases_list):
            data_gas = er3t.io.load_h5(f"data/data_flux_{gas}.h5")
            if gas == "O2":
                data_gas0 = er3t.io.load_h5("data/data_flux_O4.h5")
                ax1.fill_between(
                    data["wvl"],
                    data_gas0["f_down"][index, :],
                    data["f_down"][-1, :],
                    facecolor=colors[i],
                    lw=0.0,
                    alpha=1.0,
                ...
                data_gas0 = er3t.io.load_h5("data/data_flux_O2.h5")
                ax1.fill_between(
                    data["wvl"],
                    data_gas0["f_down"][index, :],
                    data_gas["f_down"][index, :],
                    facecolor=colors[i],
                    lw=0.0,
                    alpha=1.0,
                )
            ...
            if (gas.lower() in atm0.lev.keys()) and (gas != "O4"):
                ...
                patches_legend.append(
                    mpatches.Patch(
                        color=colors[i],
                        label=f"{gases[gas]['name']} [{gas_concentration:.1E}]",
                    )
                )
            else:
                patches_legend.append(
                    mpatches.Patch(color=colors[i], label=f"{gases[gas]['name']}")
                )

        gas_concentration = atm0.lev["air"]["data"][index]
        patches_legend.append(
            mpatches.Patch(color="gray", label=f"Air [{gas_concentration:.1E}]")
        )
