...
from er3t.core._utilities import get_doy_tag
import numpy as np
import warnings
...
def get_satname(satellite, instrument):
    ...
    if instrument.lower() == "modis" and (satellite.lower() in ["aqua", "terra"]):
        ...
        satellite.lower() in ["noaa20", "snpp", "noaa-20", "s-npp"]
    ):
        instrument = instrument.upper()
...
def cal_lon_lat_utc_geometa(
    ...
    jday0 = er3t.core._utilities.dtime_to_jday(dtime0)
...
def get_satfile_tag(
    ...
    if content is None:
        content = get_online_file(
            fname_geometa, filename=filename_geometa, fdir_save=fdir_save
        )
    # \----------------------------------------------------------------------------/#
