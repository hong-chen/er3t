...
def test_noaa20_viirs_extra():
    ...
    for i in range(Ndata):
        ...
        if len(fnames) == 1:
            ...
            lon1, lat1, jday1 = cal_lon_lat_utc_geometa(
                line,
                delta_t=360.0,
                N_along=N_along,
                N_cross=N_cross,
...
def segment_label(data_in):
    ...
    if True:
        ...
        cs = ax1.imshow(
            data_in.T, origin="lower", cmap="jet", zorder=0
        )  # , extent=extent, vmin=0.0, vmax=0.5)

        ax2 = fig.add_subplot(122)
        cs = ax2.imshow(
            seg.T, origin="lower", cmap="jet", zorder=0
        )  # , extent=extent, vmin=0.0, vmax=0.5)
...
if __name__ == "__main__":
    ...
    fname = (
        "/data/hong/mygit/les/02_retr-algm/01_cld-det/01_mul-ang/aux/cld-msk_gtruth.h5"
    )
    f = h5py.File(fname, "r")
    cld_2d = f["cloud_mask"][...]
    f.close()
