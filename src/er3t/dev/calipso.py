...
from er3t.sat.download.earthdata import final_file_check, get_command_earthdata
# import h5py
...
def get_calipso_vfm_rel(
    ...
    rel_result = []
    while start_index >= 0:
        start_index = search_content.find(
            "https://asdc.larc.nasa.gov/data/CALIPSO/LID_L2_VFM-Standard-V4-51"
        )
...
def uniform_alt_2(max_altitude, old_altitude_array):
    ...
    upper_altitudes = (np.arange(new_num_bins) + 1.0) * D_ALT
...
def regrid_lidar(alt, inMatrix, new_alt, method="linear"):
    ...
    interpFunc = interpolate.interp1d(
        alt[::-1], inMatrix[::-1, :], kind=method, axis=0, bounds_error=False
    )

    return interpFunc(new_alt)


def read_calipso_vfm(
    filename, extent, x_range=(0, 1000), y_range=(0, 20), fig_output="./tmp", plot=False
):
    """
    # This function is modified from NASA VOCAL calipso code on github:
    ...
    with HDF(filename) as product:
        ...
        for i in range(num_rows):
            unpacked_vfm[:, prof_per_row * i : prof_per_row * (i + 1)] = vfm_row2block(
                vfm[i, :]
            )
        vfm = unpacked_vfm
        ...
        if plot:
            ...
            fig.savefig(
                f"{fig_output}/calipso_lidr_flag.png", bbox_inches="tight", dpi=300
            )
