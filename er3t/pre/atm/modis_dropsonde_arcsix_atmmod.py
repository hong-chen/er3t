...
__all__ = ['modis_dropsonde_arcsix_atmmod']

class modis_dropsonde_arcsix_atmmod:
    ...
    def run(self, date, extent):
        ...
        if self.fname_ch4_clim is not None:
            self.ch4_clim(date, extent)
    ...
    def ch4_clim(self, date, extent):
        if self.fname_ch4_clim is None:
            sys.exit('Error   [atm_atmmod]: CH4 climatology file is required to calculate CH4 climatology.')
        if not os.path.exists(self.fname_ch4_clim):
            sys.exit('Error   [atm_atmmod]: CH4 climatology file %s does not exist.' % self.fname_ch4_clim)
        sat_mm = date.strftime('%m')
        # open ch climatology file
        
        with h5py.File(self.fname_ch4_clim, 'r') as f:
            # read out the data
            ch4_lon = f['longitude'][:]
            ch4_lat = f['latitude'][:]
            ...
            ch4_clim = f['ch4'][ch4_ind, :, :, :] # mixing ratio in kg/kg for levels, latitudes, and longitudes
            ...
            for i in range(num_levels):
                # Note: interp2d expects inputs in the order (x, y, z)
                # Here we use ch4_lon and ch4_lat along with the slice at level 'i'
    ...
    def interp(self, o2mix, plot=True):
        ...
        if self.fname_ch4_clim is not None:
            self.add_ch4()
            ch4_clim = self.atm_ch4['ch4_clim']['data'] # in kg/kg
            ch4_molec_weight = 16.04  # CH4 molecular weight in g/mol
    ...
    def add_insitu(self, plot=True):
        
        if plot:
            output = self.zpt_file.replace('.h5', '_interp.png')
            zpt_plot_gases(self.lev['pressure']['data'], self.lev['h2o']['data'], self.lev['co2']['data'], self.lev['o3']['data'],
                        output.replace('.png', '_gases_full_before.png'), pmin=1, ch4_vmr=self.lev['ch4']['data'])
        
        if not os.path.exists(self.fname_insitu):
            sys.exit('Error   [atm_atmmod]: In-situ data file %s does not exist.' % self.fname_insitu)
        # open in-situ data csv file
        df_insitu = pd.read_csv(self.fname_insitu)
        ...
        if not o3_nonan.any():
            sys.exit('Error   [atm_atmmod]: In-situ O3 data in %s is all NaN or negative.' % self.fname_insitu)
        if not ch4_nonan.any():
            sys.exit('Error   [atm_atmmod]: In-situ CH4 data in %s is all NaN or negative.' % self.fname_insitu)
        if not co2_nonan.any():
            sys.exit('Error   [atm_atmmod]: In-situ CO2 data in %s is all NaN or negative.' % self.fname_insitu)
        
        alt = alt[o3_nonan & ch4_nonan & co2_nonan]
        df_insitu = df_insitu[o3_nonan & ch4_nonan & co2_nonan]
        lev_below_alt = self.lev['altitude']['data'] < alt.max()
        lay_below_alt = self.lay['altitude']['data'] < alt.max()
        f_o3_insitu = interp1d(alt, df_insitu['O3_VMR_ppb'].values*1e-9, fill_value="extrapolate", kind='linear')
        self.lev['o3']['data'][lev_below_alt] = f_o3_insitu(self.lev['altitude']['data'][lev_below_alt])
        ...
        f_co2_insitu = interp1d(alt, df_insitu['CO2_VMR_ppm'].values*1e-6, fill_value="extrapolate", kind='linear')
        self.lev['co2']['data'][lev_below_alt] = f_co2_insitu(self.lev['altitude']['data'][lev_below_alt])
        self.lay['co2']['data'][lay_below_alt] = f_co2_insitu(self.lay['altitude']['data'][lay_below_alt])
