...
class sat_rep:
    ...
    # gases = ['CH4', 'CO2', 'CO', 'H2O', 'N2', 'N2O', 'O2', 'O3']
    ...
    def load_sat(self):
        ...
        f0 = Dataset('/data/hong/mygit/er3t/er3t/data/abs/reptran/reptran_solar_modis.cdf', 'r')
        ...
        wvl_int = f0.variables['wvl_integral'][:]
        print()
        print(wvl_min)
        print(wvl_max)
        print(wvl_int)
        ...
        css = f0.variables['cross_section_source'][:]
        sol = f0.variables['extra'][:]/1000.0

        iwvl = f0.variables['iwvl'][:]
        nwvl = f0.variables['nwvl_in_band'][:]
        wgt  = f0.variables['iwvl_weight'][:]
        ...
        # cs = ax1.imshow(iwvl.T, origin='lower', cmap='jet', zorder=0) #, extent=extent, vmin=0.0, vmax=0.5)
        ax1.scatter()
        # ax1.scatter(wvl, sol, s=6, c='k', lw=0.0)
        ...
        # data = f0.variables['avg_error'][:]

        # variables['avg_error']
        # variables['band_name']
        # variables['cross_section_source']
        # variables['extra']
        # variables['iwvl']
        # variables['iwvl_weight']
        # variables['max_error']
        # variables['nwvl_in_band']
        # variables['species_name']
        # variables['wvl']
        # variables['wvl_integral']
        # variables['wvlmax']
        # variables['wvlmin']
        f0.close()
