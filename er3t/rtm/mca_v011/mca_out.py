...
class mca_out_ng:
    ...
    def dump(self):
        ...
        save_h5_mca_out(self.fname, self.mca, self.abs, mode=self.mode, squeeze=self.squeeze)
...
def save_h5_mca_out(fname, mca_obj, abs_obj, mode='mean', squeeze=True):
    ...
    Input:
        ...
        mode=   : keyword argument, string, default='all', can be 'mean', 'std', 'all'
...
def read_flux_mca_out(mca_obj, abs_obj, mode='mean', squeeze=True):
    ...
    Input:
        mca_obj : positional argument, mca object, e.g., mca_obj = mcarats(...)
        abs_obj : positional argument, abs object, e.g., abs_obj = abs_16g(...)
        mode=   : keyword argument, string, default='mean', can be 'mean', 'std', 'all'
        squeeze=: keyword argument, boolen, default=True, whether to keep axis that has dimension of 1 or not, True:remove, False:keep
    ...
    if mode == 'all':
        ...
        data_dict['f_down']          = {'data':np.std(f_down              , axis=-1), 'name':'Global downwelling flux (standard deviation)' , 'units':'W/m^2/nm', 'dims_info':dims_info[:-1]}
        data_dict['f_up']            = {'data':np.std(f_up                , axis=-1), 'name':'Global upwelling flux (standard deviation)'   , 'units':'W/m^2/nm', 'dims_info':dims_info[:-1]}
        data_dict['f_down_direct']   = {'data':np.std(f_down_direct       , axis=-1), 'name':'Direct downwelling flux (standard deviation)' , 'units':'W/m^2/nm', 'dims_info':dims_info[:-1]}
        data_dict['f_down_diffuse']  = {'data':np.std(f_down-f_down_direct, axis=-1), 'name':'Diffuse downwelling flux (standard deviation)', 'units':'W/m^2/nm', 'dims_info':dims_info[:-1]}
...
def read_radiance_mca_out(mca_obj, abs_obj, mode='mean', squeeze=True):
    ...
    if mode == 'all':
        ...
        data_dict['rad'] = {'data':np.std(rad, axis=-1), 'name':'Radiance (standard deviation)' , 'units':'W/m^2/nm/sr', 'dims_info':dims_info[:-1]}
    else:
