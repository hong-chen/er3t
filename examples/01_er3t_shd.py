...
def example_01_rad_atm1d_clear_over_land(
    ...
    if plot:
        ...
        # cs = ax1.pcolormesh(np.deg2rad(vaa_2d), vza_2d, data, cmap='gist_ncar', vmin=0.0, vmax=0.2)
        # cs = ax1.pcolormesh(np.deg2rad(vaa_2d), vza_2d, data, cmap='gist_ncar', vmin=0.04, vmax=0.1)

        data = out0.data['rad']['data'][:].reshape(vaa_2d.shape)

        cs = ax1.pcolormesh(np.deg2rad(vaa_2d), vza_2d, data, cmap='jet', vmin=0.04, vmax=0.1)
...
def example_02_rad_atm1d_clear_over_ocean(
    ...
    #╭────────────────────────────────────────────────────────────────────────────╮#
    if plot:
        fname_png = f"{name_tag}-{_metadata['Function']}_{solver.lower()}_{saa:04.1f}.png"
...
def example_03_rad_atm1d_clear_over_snow(
    ...
    if plot:
        ...
        # levels = MaxNLocator(nbins=20).tick_values(0.1, 0.3)
        # cmap = plt.get_cmap('jet')
        # norm = BoundaryNorm(levels, ncolors=cmap.N, clip=True)
        # cs = ax1.pcolormesh(np.deg2rad(vaa_2d), vza_2d, data, cmap=cmap, norm=norm)
        cbar = fig.colorbar(cs, ax=ax1, shrink=0.5, aspect=30, pad=0.04, location='bottom')
...
def example_04_rad_atm1d_cloud_over_ocean(
    ...
    if plot:
        ...
        data = out0.data['rad']['data'][:].reshape(vaa_2d.shape)
        cs = ax1.pcolormesh(np.deg2rad(vaa_2d), abs(vza_2d), data, cmap='jet', vmin=0.1, vmax=0.3)
...
def example_05_rad_atm1d_clear_lambertian(
        wavelength=555.0,
        albedo=0.5,
        sza=60.0,
        saa=0.0,
    ...
    _metadata = {'Computer': os.uname()[1], 'Script': os.path.abspath(__file__), 'Function':sys._getframe().f_code.co_name, 'Date':datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
    fdir = f"{fdir0}/tmp-data/{name_tag}/{_metadata['Function']}/{albedo:04.1f}_{sza:04.1f}/{int(wavelength):04d}"
    ...
    if plot:
        ...
        data = out0.data['rad']['data'][:].reshape(vaa_2d.shape)
        cs = ax1.pcolormesh(np.deg2rad(vaa_2d), vza_2d, data, cmap='gist_ncar', vmin=0.12, vmax=0.18)
        cbar = fig.colorbar(cs, ax=ax1, shrink=0.5, aspect=30, pad=0.04, location='bottom')
...
def example_05_rad_les_cloud_3d(
    ...
    # cloud object
    cld0      = er3t.pre.cld.cld_les(fname_nc=fname_nc, fname=fname_les, coarsen=[4, 4, 5], overwrite=False)
    ...
    if False:
        ...
        ax1.set_title('SHDOM Radiance at %.2f nm (%s Mode)' % (wavelength, solver))
        plt.savefig(fname_png, bbox_inches='tight')
        plt.close(fig)
    #╰────────────────────────────────────────────────────────────────────────────╯#
...
def example_06_rad_cld_gen_hem(
    ...
    if plot:
        for ivaa in range(vaa.size):
            ...
            ax1.set_xlabel('X Index')
            ax1.set_ylabel('Y Index')
            ax1.set_title('SHDOM Radiance at %.2f nm, vaa=%.f deg (%s Mode)' % (wavelength, vaa[ivaa], solver))
            plt.savefig(fname_png, bbox_inches='tight')
            plt.close(fig)
    #╰────────────────────────────────────────────────────────────────────────────╯#
