...
def test_solar_spectra(fdir, date=datetime.datetime.now()):
    ...
    for i, wvl in enumerate(wvls):
        ...
        norm = (
            sol_fac
            / (
                abs0.coef["weight"]["data"] * abs0.coef["slit_func"]["data"][-1, :]
            ).sum()
        )
        sols[i] = (
            norm
...
def test_modis():
    download_modis_rgb(
        datetime.datetime(2015, 9, 6),
        [-2.0077, 2.9159, 48.5883, 53.4864],
        fdir=".",
        which="aqua",
        coastline=True,
    )
...
def test_viirs():
    ...
    lon_2d, lat_2d, rad_2d = grid_by_extent(
        f02.data["lon"]["data"],
        f02.data["lat"]["data"],
        f02.data["rad"]["data"].filled(fill_value=np.nan),
        extent=extent,
    )
    ...
    lon_2d, lat_2d, ref_2d = grid_by_extent(
        f02.data["lon"]["data"][logic],
        f02.data["lat"]["data"][logic],
        f02.data["ref"]["data"][logic],
        extent=extent,
        method="linear",
    )
    lon_2d, lat_2d, sfc_2d = grid_by_extent(
        f09.data["lon"]["data"],
        f09.data["lat"]["data"],
        f09.data["ref"]["data"].filled(fill_value=np.nan),
        extent=extent,
        NxNy=lon_2d.shape,
    )
    ...
    img = mpl_img.imread(
        "tmp-data/VIIRS-SNPP_rgb_2022-05-18_(-94.26,-87.21,31.86,38.91).png"
    )
    ...
    cs = ax2.imshow(
        sfc_2d.T, origin="lower", extent=extent, cmap="jet", vmin=0.0, vmax=0.5
    )
    ...
    cs = ax3.imshow(
        ref_2d.T, origin="lower", extent=extent, cmap="jet", vmin=0.0, vmax=1.0
    )
    ...
    plt.savefig(
        "%s.png" % _metadata["Function"], bbox_inches="tight", metadata=_metadata
    )
