...
def test_abs_16g(fdir):
    ...
    """
    abs_obj = er3t.pre.abs.abs_16g(
        wavelength=wavelength, fname=fname_abs, atm_obj=atm0, verbose=True
    )
    print()
...
def test_abs_rrtmg(fdir="tmp-data"):
    ...
    if False:
        for i in range(Nb):
            ...
            plt.savefig(
                "%s_band-%2.2d.png" % (_metadata["Function"], i + 1),
                bbox_inches="tight",
                metadata=_metadata,
            )
            # \--------------------------------------------------------------/#
...
def test_abs_reptran(fdir="tmp-data"):
    ...
    for i, wavelength in enumerate(wvls):
        try:
            ...
            abs1 = er3t.pre.abs.abs_rep(
                wavelength=wavelength, target="fine", atm_obj=atm0
            )
            print(i, wavelength)
            print(abs1.wvl_info)
            for j in range(alt0.size):
                coef1[i, j] = (
                    abs1.coef["abso_coef"]["data"][j, :] * abs1.coef["weight"]["data"]
                ).sum()
...
def test_abs_16g_solar(fdir):
    ...
    for i, wavelength in enumerate(wvls):
        print(wavelength)
        abs_obj = er3t.pre.abs.abs_16g(
            wavelength=wavelength, atm_obj=atm0, verbose=True
        )
        solar[i] = np.sum(
            abs_obj.coef["solar"]["data"] * abs_obj.coef["weight"]["data"]
        )
