...
def test_pha_mie_run(
    ...
    fdir = "tmp-data/%s/%s/wvl-%4.4dnm/cer-%04.1f" % (
        name_tag,
        _metadata["Function"],
        wavelength,
    ...
    abs0 = abs_16g(
        wavelength=wavelength, fname=fname_abs, atm_obj=atm0, overwrite=overwrite
    )
    # \-----------------------------------------------------------------------------/
    ...
    cld0 = cld_les(
        fname_nc=fname_nc, fname=fname_les, coarsen=[1, 1, 25], overwrite=overwrite
    )
    ...
    # /-----------------------------------------------------------------------------\
    if plot:
        fname_png = "%s-%s_%s_wvl-%4.4dnm_cer-%04.1fmm.png" % (
            name_tag,
            _metadata["Function"],
            solver.lower(),
            wavelength,
        ...
        cs = ax1.imshow(
            np.transpose(out0.data["rad"]["data"]),
            cmap="jet",
            vmin=0.0,
            vmax=0.3,
            origin="lower",
...
def figure_pha(wavelength=650.0, refs=[1, 5, 10, 15, 20]):
    ...
    plt.savefig(
        "%s_wvl-%4.4dnm.png" % (_metadata["Function"], wavelength),
        bbox_inches="tight",
        metadata=_metadata,
    )
    # \--------------------------------------------------------------/#
...
def figure_asy(wavelength=650.0, refs=[1, 5, 10, 15, 20]):
    ...
    ax1.plot(
        pha0.data["ref"]["data"], pha0.data["asy"]["data"], color="k", lw=2.0
    )  # , extent=extent, vmin=0.0, vmax=0.5)
    ax1.plot(
        pha0.data["ref"]["data"], pha0.data["asy_"]["data"], color="gray", lw=2.0
    )  # , extent=extent, vmin=0.0, vmax=0.5)
...
def figure_ssa(wavelength=650.0, refs=[1, 5, 10, 15, 20]):
    ...
    ax1.plot(
        pha0.data["ref"]["data"], pha0.data["ssa"]["data"], color="k", lw=2.0
    )  # , extent=extent, vmin=0.0, vmax=0.5)
