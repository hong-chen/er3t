...
def test_cld_les(fdir):
    ...
    dnz = dz // 0.04
    cld0 = cld_les(
        fname_nc=fname_nc, fname=fname_les, coarsing=[1, 1, dnz], overwrite=True
    )
...
def test_cld_modis_l1b(fdir):
    ...
    l2 = modis_l2(
        fname=fname_l2,
        extent=extent,
        vnames=["Solar_Zenith", "Solar_Azimuth", "Sensor_Zenith", "Sensor_Azimuth"],
    )
    print(
        l2.data["solar_zenith"]["data"].mean(), l2.data["solar_azimuth"]["data"].mean()
    )
    exit()

    lon_2d, lat_2d, ref_2d = grid_modis_by_extent(
        l1b.data["lon"]["data"],
        l1b.data["lat"]["data"],
        l1b.data["ref"]["data"][0, ...],
        extent=extent,
    ...
    yy_2stream = cal_r_twostream(
        xx_2stream, a=a0, mu=np.cos(np.deg2rad(l2.data["solar_zenith"]["data"].mean()))
    )
    ...
    for i in range(indices_x.size):
        cot_2d[indices_x[i], indices_y[i]] = xx_2stream[
            np.argmin(np.abs(yy_2stream - ref_2d[indices_x[i], indices_y[i]]))
        ]
    ...
    l1b.data["cot_2d"] = dict(
        name="Gridded cloud optical thickness", units="N/A", data=cot_2d
    )
    l1b.data["cer_2d"] = dict(
        name="Gridded cloud effective radius", units="micro", data=cer_2d
    )
...
def test_cld_modis_l2(fdir):
    ...
    cs2 = ax2.scatter(
        l2.data["lon_5km"]["data"], l2.data["lat_5km"]["data"], c=angle, s=0.1
    )
    plt.colorbar(cs2)
...
def test_sfc_modis_09a1(fdir):
    ...
    filename_tag = "h11v05"
    fnames = download_modis_https(
        date, "6/MOD09A1", filename_tag, day_interval=8, fdir_out="data", run=True
    )
